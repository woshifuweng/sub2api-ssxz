...
- P1 product/operations: about 39%. Completed or staged slices: image-model alias display clarity, user-shell lint baseline cleanup, first image history/download feedback hardening, staging catalog exposure verification, API Key third-party access copy/safety polish, usage explanation-state clarity, profile TOTP failure clarity, masked API-key configuration guards, `/app/usage` DTO/balance-refresh boundary verification, Google/Gemini-compatible API-key auth restriction parity, production-deployed usage-billing no-overdraft safety, and production-deployed bounded generic/Gemini token request cost gating. Remaining large P1 loops: controlled production real-generation acceptance, broader usage/balance workflow verification, full API Key lifecycle/security verification, and admin/ops hardening.
- Distance to P2: about 61% of P1 remains. Do not prioritize P2 visual polish until P1 loops have evidence.
...
- `/app/usage` real balance, monthly trend, detail data, and failure/no-charge explanation accuracy:
  ...
  - PR #214 deployed bounded generic/Gemini token request estimated-cost eligibility checks to staging and production; WebSocket/unbounded spend-cap coverage remains open.
