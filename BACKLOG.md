...
- 2026-07-08: P1 commercial-mainline status is roughly 96%. Production was previously deployed to main `26639dbe` with user workspace, API Key, usage, channel status, purchase/order/redeem/profile, and admin smoke checks passing. PR #346 merged the CCS import platform guard, and PR #347 merged a test-only API Key handoff locale assertion sync.
...
- 2026-07-08: production no-provider customer handoff gate passed for public user pages, ordinary/admin login, ordinary user groups/channels/API Keys/usage/redeem visibility, ordinary-user admin 403, and admin users/usage/redeem/payment/groups/dashboard read paths. `CUSTOMER_HANDOFF_GATE.md` is the tracked handoff checklist. No provider calls or write operations were performed.
- 2026-07-08: controlled production GPT handoff validation added a low-quota customer key and verified `gpt-5.5` plus `gpt-5.4-mini` through the SSXZ production gateway, with usage traceable to the GPT upstream account. The evaluated Gemini-compatible upstream was disabled after upstream quota/high-demand errors, so Gemini should not be recommended on this handoff path yet.
- 2026-07-08: PR #354 through PR #356 were merged and deployed as production main `798412473`. The batch aligned account menu, dashboard, purchase/order, account-balance, and third-party handoff wording around the P1 commercial mainline.
- 2026-07-08: staging and production now both run SHA-256 `b1593aa8a6aa4da584690a1dee44bb63a0225c682336c7bc9cc6bf1b1f49429c`. Staging backup: `/opt/sub2api/backups/staging-before-main-798412473-20260708-070715-sub2api-staging`. Production backup: `/opt/sub2api/backups/production-before-main-798412473-20260708-071029-sub2api`.
- 2026-07-08: production HTTPS smoke after main `798412473` returned HTTP 200 for `/health`, `/home`, `/login`, `/register`, `/app/dashboard`, `/app/keys`, `/app/usage`, `/app/channel-status`, `/app/purchase`, `/app/orders`, `/app/redeem`, `/app/profile`, `/app/chat`, `/app/image`, and `/api/v1/settings/public`; `/v1/models` without an API key returned HTTP 401. No real provider call, database migration, Nginx change, payment logic change, or provider-routing change was part of the release.
...
- P1 product/operations: about 96%. API Key lifecycle, group binding, one-time full-key reveal, masked-key safety, CCS import guard, usage, purchase, orders, redeem, admin diagnostics, and one controlled production GPT relay handoff now have evidence. Remaining P1 loops are mostly real customer client friction, recharge/order/redeem release evidence, and small fixes found during onboarding.
- Distance to P2: about 4% of P1 remains. Do not prioritize P2 visual polish until customer handoff, recharge/order/redeem, and admin-support evidence are stable enough for controlled low-quota customers.
