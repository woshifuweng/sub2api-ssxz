...
| 层 | 数 |
|---|---|
| 分家点以来我们改过（剔构建垃圾） | **1361** |
| 其中非测试 | 1014 |
| 减去 `backend/ent/` **生成代码**（`// Code generated by ent, DO NOT EDIT.`，重放靠重新生成而非手抄） | −127 |
| **→ 需手工重放** | **887** |
| ├ A 类：只我们改过（直接搬，无冲突） | **399** |
| └ B 类：两边都改过（必须语义重叠，不能整文件覆盖） | **488** |

按 package 穷尽归类（top）：`service` 201 · `components` 141 · `handler` 96 ·
`views` 91 · `repository` 44 · `server` 41 · `api` 27 · `migrations` 21。

客户可见 UI 重放清单：**232 个**（只我们改 84 / 两边都改 148）。

⚠️ **Codex 的 12 个类目只覆盖了 275/994**，剩下 719 个不是"不属于 SSXZ"，
是关键词分类法漏了。**账本必须按 package 穷尽归类，不能按关键词**——
否则漏掉的正好是 `service`/`handler` 这些最核心的。
