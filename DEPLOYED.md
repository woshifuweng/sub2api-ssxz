...
> 最后核验：2026-08-30 v0.1.183 SSXZ `.8` 用户使用记录与 API Key 完整性修复、部署及生产页面验收后（核验方法见文末，任何人可自行复跑）

## 当前生产摘要（2026-08-30 v0.1.183 SSXZ 合并版）

- 生产版本：`0.1.183-ssxz.20260830.8`
- 源码 commit：`0e0016ae4b863cffbe658e34cd2bb22be7f403b9`（远端分支 `codex/upstream-v183-ssxz-20260830`）
- 线上二进制：MD5 `97962a0d9e96691ef7f181cc5ad7cdc5`，SHA256 `ec9761c3bacd5d1e44ece58d207c013758f0b262d31918395c71d71088830dab`，`120,975,520` bytes。
- 上游底座：Wei-Shaw/Sub2API `v0.1.183`；以前述底座为主，迁移并保留 SSXZ 品牌、业务服务、安全加固、权限分流和生产配置，没有整包覆盖生产数据或配置。
- 前端入口：`/assets/index-Bj6VBVYZ.js`；嵌入式前端 `242` 文件 / `7,525,512` bytes。
...
- 回滚基线：`.8` 发布前生产二进制与配置快照 `/opt/sub2api/backups/v183r8-prod-before-20260830-134357`，staging 快照 `/opt/sub2api/backups/v183r8-staging-before-20260830-134341`；完整数据库回滚点仍为 `/opt/sub2api/backups/v183r2-before-20260830-1200`。
- 本轮发现并修复：保留 SSXZ 自有 UI；用户使用记录恢复 Sub2 原生完整明细能力并以 SSXZ 主题渲染，直接展示 API Key、模型、推理、接口、IP、分组、类型、计费、Token、费用、耗时、时间和定位码；API Key 页收口分组与用量布局；取消会遮住相邻字段的水平固定列；桌面表内横滑与手机卡片布局均通过。
- 真实链路：staging 与生产均使用既有企业 Key 调用 `POST /v1/responses` / `gpt-5.4-mini` 返回 HTTP 200；管理员运营汇总真实数据已加载；生产服务无重启、无持续错误。
- 门禁：前端 `238` 个测试文件 / `1646` 条用例、41 条页面定向用例、类型检查、局部 ESLint 和生产构建通过；本轮仅改嵌入式前端，后端在同一 v0.1.183 基线上的 `go test ./...`、`go vet ./...` 结果继续有效。生产深浅色、1440px 桌面与 390px 手机真人验收覆盖用户使用记录和 API Key，控制台零 warning/error。
- 未改动数据库、OAuth、账号、余额、分组授权和生产计费倍率；本轮没有为 UI 验收创建 Key 或发起收费模型请求。磁盘使用率为 `88%`，不阻断本次发布，但已成为近期必须清理或扩容的运维风险。
- 回滚边界：`.8` 没有新增 schema；二进制可直接恢复 `.8` 发布前快照。迁移 231 仍向后兼容，二进制回滚不会自动撤销；不要用旧数据库覆盖部署后的新增业务数据。
...
```
"version":"0.1.183-ssxz.20260830.8"
```
