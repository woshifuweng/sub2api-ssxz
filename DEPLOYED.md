...
- 预发布版本：`0.1.183-ssxz.20260831.15`；运行代码 commit `1f7beef9bcc7bd42c80daf29d11a0d9692888aa6`，隔离验收脚本修正 commit `2e68d598b1d53862b91c1c52a33675409edaa2af`，远端分支 `fork/codex/staging-isolation-20260831`。
...
- 生产版本：`0.1.183-ssxz.20260830.12`
- 源码 commit：`b8cc97e50652af4c71d96ede863a90092e4811c5`（PR `#361` 已合并，远端分支 `codex/upstream-v183-ssxz-20260830` 已同步到同一 commit）
- 线上二进制：MD5 `73d36a8952cbe840214fa9abb46f19c1`，SHA256 `515733ebf87f6c73b8badb3d7b1e65a064d97e689eb53718d975a37b4587c398`，`120,979,616` bytes；生产与 staging 哈希一致。
- 上游底座：Wei-Shaw/Sub2API `v0.1.183`；以前述底座为主，迁移并保留 SSXZ 品牌、业务服务、安全加固、权限分流和生产配置，没有整包覆盖生产数据或配置。
- 前端入口：`/assets/index-D3J1oQbb.js`。
...
- 回滚基线：`.12` 发布前生产、staging 二进制、配置、systemd、PostgreSQL 快照与校验清单位于 `/opt/sub2api/backups/v183r12-before-20260830-190853`；校验清单已通过，配置与发布前逐字节一致。此前 `.11` 及更早快照继续保留。
- 备份去重：2026-08-30 17:08 CST 对 `/opt/sub2api/backups` 做 SHA256、权限和属主一致性校验后，将 28 组、36 个完全相同的旧备份文件原子替换为硬链接，释放约 `4.20GB`；所有原路径和回滚脚本仍可用，唯一备份未删除。记录清单为 `/opt/sub2api/backups/DEDUP-MANIFEST-20260830-170830.json`。系统盘由 `89%` 降至 `84%`，可用空间由约 `9.9GB` 增至约 `14GB`，备份目录由 `18GB` 降至 `14GB`。
- 本轮发现并修复：保留 SSXZ 自有 UI；用户使用记录用组合列减少拥挤但保留 API Key、模型、推理、接口、IP、分组、类型、计费、Token、费用、耗时、时间和定位码，手机改为完整卡片；API Key 默认合并为 6 列，分组不再遮住用量，可选列仍可开启和排序；后台顶部统一为 56px 且标题、版本号、分割线对齐；邀请返利等页面不再出现翻译键；版本弹窗能识别 SSXZ 后缀；主站“图片工作台”改为直接打开正式独立域名，`/app/image` 兼容入口也会跳转，不再把 API Key 放入 URL。
- 真实链路与页面：既有企业 Key 的 `POST /v1/responses` / `gpt-5.4-mini` 双环境 HTTP 200 结果继续有效；本轮在生产用已登录 Chrome 从主站实际点击“图片工作台”，新标签正确打开 `https://image.ssxzapi.com/`，地址栏无查询参数和 API Key；再点击“返回仪表盘”能回到已登录主站。图片主域、备用域、API 健康页和主站关键路由均为 HTTP 200，未带 Key 的模型接口为 401。
- 门禁：前端 `239` 个测试文件 / `1673` 条用例、类型检查、ESLint、生产构建和 `git diff --check` 全部通过；本轮只改嵌入式前端，后端没有代码差异。CI 前端、前端安全、后端安全和 Shell 门禁通过；Go lint 与 Go 单测仍受同一生产基线已有问题影响，本轮没有扩大范围去修改无关后端。生产与 staging 均为 `active`、`NRestarts=0`，发布后错误级日志为 0。
- 安全收口：从 systemd 中移除了两个遗留的明文密钥配置片段；其中旧管理员引导密码已在服务器端确认不匹配任何当前管理员密码，移除后现有登录不受影响。未在日志或文档中记录任何密钥值。
- 未改动数据库 schema、OAuth、账号、余额、真实生产数据、分组授权和生产计费倍率；没有为本轮页面验收创建 Key 或修改企业配置。
- 回滚边界：`.12` 没有新增 schema；可用上述快照恢复二进制、配置和服务。不要用旧数据库覆盖部署后的新增业务数据。

## 图片工作台 v0.7.8 独立域名与真实链路验收（2026-08-30）

- 代码基线：以 `CookSleep/gpt_image_playground` v0.7.8 为底座，保留 SSXZ 品牌、返回主站入口和既有性能优化；上游合并 PR `#1` 的结果继续有效。本轮发布加固通过 PR `#2`、`#3` 合入 fork `main`，最终 merge commit 为 `47be9b4a45b6cd545edc2d70d0cf08333bf49250`。
- 正式入口：`https://image.ssxzapi.com/`；保留 `https://gpt-image-playground.dilciamaribelvenegas873.workers.dev/` 作为独立回退入口。Cloudflare Worker 生产版本 ID：`c66ac136-79f4-4e20-b03b-ff4c54593a1a`。
- 主站入口：主站侧 PR `#361` 已将侧边栏“图片工作台”和兼容路由 `/app/image` 统一指向正式域名；生产静态资源与本地通过门禁的构建哈希一致，并确认线上资源不存在 `apiKey=` URL 参数。
- 持续观察：已建立 24 小时只读巡检，每两小时检查主域、备用域、API、生产/预发布服务状态、重启和错误日志；巡检不会发送付费图片生成请求，也不会修改业务数据。
- 发布门禁：新增 GitHub Actions，在 PR 和 `main` 推送时执行依赖锁定安装、518 条前端用例、生产构建和中高危依赖审计；PR `#2`、`#3` 的门禁均通过后才合并。`preview_urls` 已关闭，`workers.dev` 回退域名保持开启。
- 跨域调用：生产配置只允许上述正式域名和回退域名访问 API，并启用凭据支持；两个来源的预检请求均为 HTTP 204，返回的允许来源与请求来源精确一致，没有使用通配符。改动前配置备份为 `/opt/sub2api/backups/config-pre-image-cors-20260830-184019.yaml`，SHA256 为 `8434c9c199d9ca7b25d2b2cf25c7f1e5274ba7e379918a225e0b2f77cd80c926`。
- 真实验收：在 Chrome 中从正式域名发起 `gpt-image-2` 生成，API 返回 HTTP 200，图片实际渲染成功；测试前后账户余额由 `$484.78270026` 变为 `$484.63270026`，该次扣费 `$0.15`。连同此前服务器直连验证，本轮真实生成共 2 次、总扣费 `$0.30`；复用的低额度测试 Key 验收后已恢复为禁用状态。
- 生产健康：`sub2api` 为 `active`，本机和公网 `/health` 均为 HTTP 200，修复后日志无持续错误；正式域名和回退域名首页均为 HTTP 200。
- 异常记录：原子替换跨域配置时文件属主曾短暂变为 `root:root`，服务账户无法读取配置，造成约 20 秒 HTTP 502。已立即恢复为 `root:sub2api`、权限 `640` 并完成服务、健康检查、日志和跨域回归验证；当前无遗留异常。
- 数据边界：没有修改数据库 schema、生产账号、分组、倍率或业务数据；仅产生上述两笔明确的真实测试费用，并短暂启用后重新禁用了既有测试 Key。
...
```
"version":"0.1.183-ssxz.20260830.12"
```
...
热态复测已确认：服务连续运行后，账号 32 三次为 3.384/3.385/3.526 秒，账号 38 三次为
3.385/3.391/3.386 秒，Kedaya `gpt-5.5` 为 3.389 秒；因此结论是**重启后冷启动**，
连接池空、无复用 TLS 会话、DNS 未缓存等因素导致首轮变慢，并非真实延迟回归。
