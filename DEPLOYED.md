...
> 最后核验：2026-08-30 v0.1.183 SSXZ `.11` 用户使用记录、API Key、后台顶部比例和版本提示修复，完成 staging/生产部署及真实页面验收后（核验方法见文末，任何人可自行复跑）

## 当前生产摘要（2026-08-30 v0.1.183 SSXZ 合并版）

- 生产版本：`0.1.183-ssxz.20260830.11`
- 源码 commit：`c8f2371298505ee6b3b73c8aa82a678b5b6d6c3b`（远端分支 `codex/upstream-v183-ssxz-20260830` 已同步到同一 commit）
- 线上二进制：MD5 `9fee4c12fdfa2eeb16aab12ec57b680f`，SHA256 `80f3f3e63f7881a7fda15eab21c2e033f2dc97078a8ec64ffa212de2e73c5441`，`120,979,616` bytes；生产与 staging 哈希一致。
- 上游底座：Wei-Shaw/Sub2API `v0.1.183`；以前述底座为主，迁移并保留 SSXZ 品牌、业务服务、安全加固、权限分流和生产配置，没有整包覆盖生产数据或配置。
- 前端入口：`/assets/index-C1B_YC-s.js`。
...
- 回滚基线：`.11` 发布前生产、staging 二进制、配置、systemd、PostgreSQL 快照与校验清单位于 `/opt/sub2api/backups/v183r11-before-20260830-163803`，目录内含 `rollback-fast.sh`；此前 `.10`、`.9`、`.8` 快照继续保留。
...
- 本轮发现并修复：保留 SSXZ 自有 UI；用户使用记录用组合列减少拥挤但保留 API Key、模型、推理、接口、IP、分组、类型、计费、Token、费用、耗时、时间和定位码，手机改为完整卡片；API Key 默认合并为 6 列，分组不再遮住用量，可选列仍可开启和排序；后台顶部统一为 56px 且标题、版本号、分割线对齐；邀请返利等页面不再出现翻译键；版本弹窗能识别 SSXZ 后缀，不再把当前 `0.1.183` 误报为可更新，同时保留 rc/alpha/beta 到正式版的真实更新提示。
- 真实链路与页面：既有企业 Key 的 `POST /v1/responses` / `gpt-5.4-mini` 双环境 HTTP 200 结果继续有效；本轮在生产用已登录用户和管理员实际点击使用记录、API Key、邀请返利、后台仪表盘和版本弹窗，筛选重置、列设置、排序、展开、获取地区和中英文切换均通过。
- 门禁：前端 `239` 个测试文件 / `1672` 条用例、类型检查、ESLint、生产构建和 `git diff --check` 全部通过；本轮仅改嵌入式前端，后端在同一 v0.1.183 基线上的 `go test ./...`、`go vet ./...` 结果继续有效。生产 1920px/1440px 桌面与 390px 手机验收通过：页面本身均无横向溢出，API Key 默认表格无内部横滑，手机字段完整。
- 未改动数据库 schema、OAuth、账号、余额、真实生产数据、分组授权和生产计费倍率；没有为本轮页面验收创建 Key 或修改企业配置。
- 回滚边界：`.11` 没有新增 schema；可执行上述快照目录内的快速回滚脚本恢复二进制与服务。不要用旧数据库覆盖部署后的新增业务数据。
...
```
"version":"0.1.183-ssxz.20260830.11"
```
