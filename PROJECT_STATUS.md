...
## Production Release Recorded On 2026-06-27 For Current Main `8324754`

| Area | Result | Evidence |
| --- | --- | --- |
| Deployment | Completed after explicit user approval | Current main `832475454e7eac9feb55d66a2645ae0a470034a0` was deployed to `sub2api.service` after the user explicitly approved deploying `8324754` to production. |
| Binary | Production updated; staging unchanged | Production `/opt/sub2api/sub2api` ran SHA-256 `b5da079f49da94cfb57873de6d4fada5d170b87be310ab0633ecafc53f2d877b`. Staging `/opt/sub2api/sub2api-staging` remained SHA-256 `7086880a79e22900e4d65ca4f1cc715f6968c77fa22ec92a5c85bced8dfbcd5e`. |
| Backup | Created before replacement | `/opt/sub2api/backups/production-before-main-8324754-20260627-222309/sub2api`, with backup SHA-256 `7fb45509c5fb6d74a5cc8ab88530f78f4e34dd5a88b177fe31c178b3f034afa0`. |
| Production smoke | Public routes responded | `https://api.ssxzapi.com/app/chat`, `/app/image`, `/app/usage`, `/app/keys`, `/app/profile`, and `/api/v1/settings/public` returned HTTP 200 after deployment. |
| Authenticated no-provider check | Read-only checks passed | Ordinary and admin login succeeded with roles `user` and `admin`. Ordinary image history and usage dashboard were readable. Ordinary production image model count remained `0`. |
| Scope | Frontend image-history/download feedback reached production | This production release carried the #192 frontend feedback hardening to the real site. It did not change Nginx, database, payment/ledger, or provider routing, and it did not call a real provider. |
| Remaining gate | Production real image generation still blocked | Production ordinary users still have no selectable image models, so controlled production image-generation acceptance cannot start until production image-model availability is intentionally configured and approved. |

## Staging Release Recorded On 2026-06-27 For PR #194

| Area | Result | Evidence |
| --- | --- | --- |
| Deployment | Completed on staging only | PR #194 was merged to main at `c208d51a7` and deployed only to `sub2api-staging.service`. |
| Binary split | Staging and production are different binaries | Staging `/opt/sub2api/sub2api-staging` ran SHA-256 `9018e284ee6a80d8fd717ddd49b1457e65fd278025b76377b5fa5b9855d66869`. Production `/opt/sub2api/sub2api` remained SHA-256 `b5da079f49da94cfb57873de6d4fada5d170b87be310ab0633ecafc53f2d877b`. |
| Backup | Created before replacement | `/opt/sub2api/backups/staging-before-pr194-c208d51a7-20260627-225321-sub2api-staging`, with backup SHA-256 `7086880a79e22900e4d65ca4f1cc715f6968c77fa22ec92a5c85bced8dfbcd5e`. |
| Staging smoke | Public routes responded | `/app/chat`, `/app/image`, `/app/usage`, `/app/keys`, `/app/profile`, and `/api/v1/settings/public` returned HTTP 200 on staging port `18080`. |
| Image model catalog | Ordinary staging user can select real-channel image models | Ordinary-user `/api/v1/channels/available` exposed 4 `image_generation` models, including `gpt-image-2`, all with `provider=openai-compatible-images` and `model_catalog_source=real_channel`; non-real image model count was `0`. |
| Scope | Catalog metadata exposure only | #194 touched available-channel catalog metadata and tests. It did not change Nginx, database, payment/ledger, provider routing, or production config, and it did not call a real provider. |
| Remaining gate | Production image acceptance still open | Production remains on `8324754`, so production image creation, storage, usage/billing, history, and download are still not accepted. |
...
| Image generation real upstream | Staging verified, production authenticity and alias-display guards deployed | One `gpt-image-2` OpenAI-compatible path works in staging. Production was deployed on 2026-06-21 after explicit approval, PR #184 was deployed on 2026-06-27 after explicit approval, and PR #188 was deployed on 2026-06-27 after explicit approval. PR #186 confirmed staging image selection is driven by the server-side OpenAI-compatible image allowlist, PR #188 clarified Gemini-named aliases as OpenAI-compatible image-route aliases, and PR #194 verified ordinary-user staging catalog exposure for real-channel image models. Real production image generation, storage, billing, history, and download still require controlled acceptance checks. |
