...
| Affiliate / invite rebate | `/app/affiliate`, `/admin/affiliates`, register `?aff=` / `?affiliate=`, `backend/internal/repository/usage_billing_repo.go`, `backend/internal/repository/affiliate_repo.go` | Existing end-to-end chain covers inviter code, register binding, idempotent rebate accrual from successfully charged model consumption, transferable quota, invitee list, and admin custom rates. Balance recharge itself does not accrue rebate. Keep hidden unless `affiliate_enabled` is on; do not design a separate promoter system. |
...
| --- | --- | --- |
| `/app`, `/app/chat` | Connected, canonical chat entry | Unified workspace/chat path exists. `/app/chat` is the ordinary-user new chat entry and brand-home destination. PR #228 was deployed to staging only and confirmed the frontend build no longer blocks the chat workspace backend gate there. The UX is not final. |
...
## Staging And Production Release Recorded On 2026-06-29 For Main `57eccdd3` / PR #219

| Area | Result | Evidence |
| --- | --- | --- |
| Deployment | Completed on staging and production | PR #219 was merged to main at merge commit `57eccdd3`, deployed first to `sub2api-staging.service`, then to `sub2api.service` after CI and staging smoke passed. |
| Binary | Production and staging aligned | Production `/opt/sub2api/sub2api` and staging `/opt/sub2api/sub2api-staging` both run SHA-256 `6b6a82785d1584a5ab5d917d0d786de018ff1879dd50342a19ed5f46ae604411`. |
| Backups | Created before replacement | Staging backup: `/opt/sub2api/backups/staging-before-pr219-57eccdd3a-20260629-065352-sub2api-staging`. Production backup: `/opt/sub2api/backups/production-before-pr219-57eccdd3a-20260629-065440/sub2api`. |
| Production smoke | Public routes responded | `https://api.ssxzapi.com/app/chat`, `/app/image`, `/app/usage`, `/app/keys`, `/app/profile`, and `/api/v1/settings/public` returned HTTP 200; `/v1/models` without an API key returned HTTP 401. |
| Runtime health | Services active | `sub2api.service` and `sub2api-staging.service` were active after restart. |
| Token-request billing gate | No-cap token requests get a pre-provider safety-budget check | PR #219 adds a conservative backend cost estimate when token requests omit a positive output cap, then reuses the existing cost-aware billing eligibility checks before account/provider selection on generic gateway and OpenAI-compatible gateway paths. |
| Scope | Backend billing safety release | No database migration, Nginx change, payment file change, provider-routing change, production config change, or real-provider call was part of the release. |
| Remaining gate | Full spend-cap design is not complete | This release closes the immediate positive-balance/no-cap provider-dispatch risk, but it is not a full reservation, pre-charge, or streaming ledger-hold system. It also does not count as real-provider production image-generation acceptance. |

## Staging Release Recorded On 2026-06-29 For Main `87504f2096b0` / PR #228

| Area | Result | Evidence |
| --- | --- | --- |
| Deployment | Completed on staging only | PR #228 was merged to main at merge commit `87504f2096b0` and deployed only to `sub2api-staging.service` from GHCR image `ghcr.io/woshifuweng/sub2api:87504f2096b0`. |
| Binary | Staging updated, production untouched | Staging `/opt/sub2api/sub2api-staging` runs SHA-256 `5a8da633aa7a165a4dc365370fffa4303ca1ba6087216f0e822948bec6ecd0b3`; production was not restarted or replaced during this validation. |
| Backup | Created before replacement | Staging backup: `/opt/sub2api/backups/staging-before-main-87504f2096b0-20260629-180231-sub2api-staging`. |
| Staging smoke | Public routes responded | `/health`, `/app/chat`, `/app/image`, `/app/usage`, `/app/keys`, and `/app/profile` returned HTTP 200 on staging; `/v1/models` without an API key returned HTTP 401. |
