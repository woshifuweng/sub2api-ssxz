...
type AccountHandler struct {
	...
	accountListCache        *snapshotCache
}
...
func NewAccountHandler(
	...
	return &AccountHandler{
		...
		accountListCache:        newSnapshotCache(5 * time.Second),
	}
}
...
func (h *AccountHandler) scoreOpenAIAccountSchedulerPool(ctx context.Context, accounts []service.Account, loadMap map[int64]*service.AccountLoadInfo) map[int64]AccountSchedulerScore {
	...
	minPriority, maxPriority := openAIAccounts[0].Priority, openAIAccounts[0].Priority
	maxWaiting := 1
	...
	// These are the upstream default scheduler weights. The list score is an
	// optional diagnostic snapshot; live scheduling continues to use the
	// service-owned runtime weights and admission path.
	const (
		priorityWeight = 1.0
		loadWeight     = 1.0
		queueWeight    = 0.7
	...
	result := make(map[int64]AccountSchedulerScore, len(openAIAccounts))
	for _, account := range openAIAccounts {
		priorityFactor := 1.0
		if maxPriority > minPriority {
			priorityFactor = 1 - float64(account.Priority-minPriority)/float64(maxPriority-minPriority)
		}
		loadFactor, queueFactor := 1.0, 1.0
		if load := loadMap[account.ID]; load != nil {
			loadRate := float64(load.LoadRate) / 100
			if loadRate < 0 {
				loadRate = 0
			} else if loadRate > 1 {
				loadRate = 1
			}
			loadFactor = 1 - loadRate
			queueRate := float64(load.WaitingCount) / float64(maxWaiting)
			if queueRate < 0 {
				queueRate = 0
			} else if queueRate > 1 {
				queueRate = 1
			}
			queueFactor = 1 - queueRate
		}
		result[account.ID] = AccountSchedulerScore{
			BaseScore:             priorityWeight*priorityFactor + loadWeight*loadFactor + queueWeight*queueFactor + errorWeight + ttftWeight*0.5,
...
// BatchDelete handles deleting multiple accounts.
// POST /api/v1/admin/accounts/batch-delete
func (h *AccountHandler) BatchDelete(c *gin.Context) {
	h.batchDeleteAccounts(c)
}
...
/* The mechanically interleaved model handlers are retained temporarily below
for migration archaeology. The valid semantic union follows this block.
...
func (h *AccountHandler) GetAvailableModelsGateway(c gatewayctx.GatewayContext) {
	...
	case service.PlatformGrok:
		modelIDs := mappingKeys(account.GetModelMapping())
		if len(modelIDs) == 0 {
			response.SuccessContext(gatewayJSONResponder{ctx: c}, xai.DefaultModels())
			return
		}
		...
		response.SuccessContext(gatewayJSONResponder{ctx: c}, models)
		return
...
// SyncUpstreamModels syncs live supported models from an account's upstream.
// POST /api/v1/admin/accounts/:id/models/sync-upstream
func (h *AccountHandler) SyncUpstreamModels(c *gin.Context) {
	accountID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
...
// RefreshModels refreshes an account's fetched model list.
// POST /api/v1/admin/accounts/:id/models/refresh
func (h *AccountHandler) RefreshModels(c *gin.Context) {
	h.RefreshModelsGateway(gatewayctx.FromGin(c))
}
...
// SyncUpstreamModelsPreview syncs models using credentials that are not yet saved.
// POST /api/v1/admin/accounts/models/sync-upstream-preview
func (h *AccountHandler) SyncUpstreamModelsPreview(c *gin.Context) {
	var req struct {
		Platform string `json:"platform" binding:"required"`
...
// SetPrivacy sets privacy for one OpenAI or Antigravity OAuth account.
// POST /api/v1/admin/accounts/:id/set-privacy
func (h *AccountHandler) SetPrivacy(c *gin.Context) {
	accountID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
