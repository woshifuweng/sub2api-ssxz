...
func (h *ChannelHandler) SyncPricingModels(c *gin.Context) {
	h.SyncPricingModelsGateway(gatewayctx.FromGin(c))
}

func (h *ChannelHandler) SyncPricingModelsGateway(c gatewayctx.GatewayContext) {
	platform := strings.ToLower(strings.TrimSpace(c.QueryValue("platform")))
	if platform == "" {
		response.ErrorFromContext(gatewayJSONResponder{ctx: c}, infraerrors.BadRequest("MISSING_PARAMETER", "platform parameter is required").
	...
	if !ok {
		response.ErrorFromContext(gatewayJSONResponder{ctx: c}, infraerrors.BadRequest("UNSUPPORTED_PLATFORM",
	...
	response.SuccessContext(gatewayJSONResponder{ctx: c}, gin.H{"models": models})
}
