...
func (h *OpsHandler) ListIngressRejects(c *gin.Context) {
	...
	if filter.UserID, err = parseOptionalPositiveQueryID(c, "user_id"); err != nil {
		response.BadRequest(c, err.Error())
		return
	}
	if filter.APIKeyID, err = parseOptionalPositiveQueryID(c, "api_key_id"); err != nil {
		response.BadRequest(c, err.Error())
		return
	}
...
func parseOptionalPositiveQueryID(c *gin.Context, name string) (*int64, error) {
