...
func (h *AuthHandler) LinuxDoOAuthCallbackGateway(c gatewayctx.GatewayContext) {
	...
	if compatEmailUser == nil && !emailVerificationRequired && !forceEmailOnSignup {
		...
		tokenPair, user, err := h.authService.LoginOrRegisterOAuthWithTokenPair(
			c.Request().Context(), email, username, "", "", "linuxdo",
		)
		if err == nil {
			if err := applyPendingOAuthBinding(
				...
				redirectOAuthErrorGateway(c, frontendCallback, "session_error", "failed to bind oauth identity", "")
				return
			}
			h.authService.RecordSuccessfulLogin(c.Request().Context(), user.ID)
			clearOAuthPendingSessionCookieGateway(c, secureCookie)
			clearOAuthPendingBrowserCookieGateway(c, secureCookie)
			redirectOAuthTokenPairGateway(c, frontendCallback, tokenPair, redirectTo)
			return
		}
...
func redirectOAuthTokenPairGateway(c gatewayctx.GatewayContext, frontendCallback string, tokenPair *service.TokenPair, redirectTo string) {
	fragment := url.Values{}
	if tokenPair != nil {
