...
func TestAvailableChannelDTOExposesDisplayMetadata(t *testing.T) {
	maxInput := 200000
	maxOutput := 128000
	groups := filterUserVisibleGroups([]service.AvailableGroupRef{{
		ID:          11,
		Name:        "Claude 满血池",
		Description: "满血高质量",
		Platform:    "anthropic",
	...
	require.Equal(t, "满血高质量", groups[0].Description)
	...
	}}, map[string]struct{}{"anthropic": {}}, nil, 0)
	require.Len(t, models, 1)
	require.Equal(t, &maxInput, models[0].ContextLength)
	require.Equal(t, &maxOutput, models[0].MaxOutputTokens)
}
