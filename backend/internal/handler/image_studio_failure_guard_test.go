...
func (u *imageStudioGatewayUpstream) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, _ *tlsfingerprint.Profile) (*http.Response, error) {
...
func TestImageStudioGenerateGateway_IdempotencyReplayDoesNotCallUpstreamOrDuplicateHistory(t *testing.T) {
	...
	first := callGenerate()
	second := callGenerate()
	t.Logf("first status=%d body=%s second status=%d body=%s", first.Code, first.Body.String(), second.Code, second.Body.String())
