...
func (h *PaymentWebhookHandler) EasyPayNotify(c *gin.Context) {
	h.EasyPayNotifyGateway(gatewayctx.FromGin(c))
}

func (h *PaymentWebhookHandler) EasyPayNotifyGateway(c gatewayctx.GatewayContext) {
	h.handleNotifyGateway(c, payment.TypeEasyPay)
}
...
func (h *PaymentWebhookHandler) AlipayNotify(c *gin.Context) {
	h.AlipayNotifyGateway(gatewayctx.FromGin(c))
}

func (h *PaymentWebhookHandler) AlipayNotifyGateway(c gatewayctx.GatewayContext) {
	h.handleNotifyGateway(c, payment.TypeAlipay)
}
...
func (h *PaymentWebhookHandler) WxpayNotify(c *gin.Context) {
	h.WxpayNotifyGateway(gatewayctx.FromGin(c))
}

func (h *PaymentWebhookHandler) WxpayNotifyGateway(c gatewayctx.GatewayContext) {
	h.handleNotifyGateway(c, payment.TypeWxpay)
}

// StripeWebhook handles Stripe webhook events.
// POST /api/v1/payment/webhook/stripe
func (h *PaymentWebhookHandler) StripeWebhook(c *gin.Context) {
	h.StripeWebhookGateway(gatewayctx.FromGin(c))
}

func (h *PaymentWebhookHandler) StripeWebhookGateway(c gatewayctx.GatewayContext) {
	h.handleNotifyGateway(c, payment.TypeStripe)
}

// handleNotify is the shared logic for all provider webhook handlers.
func (h *PaymentWebhookHandler) handleNotify(c *gin.Context, providerKey string) {
	h.handleNotifyGateway(gatewayctx.FromGin(c), providerKey)
}

func (h *PaymentWebhookHandler) handleNotifyGateway(c gatewayctx.GatewayContext, providerKey string) {
	var rawBody string
	if c.Request().Method == http.MethodGet {
		...
		if err != nil {
			...
			writeGatewayString(c, http.StatusBadRequest, "failed to read body")
			return
		}
		rawBody = string(body)
	}
	...
	if err != nil {
		...
		if providerKey == payment.TypeWxpay {
			writeGatewayString(c, http.StatusBadRequest, "verify failed")
			return
		}
		writeSuccessResponseGateway(c, providerKey)
		return
	}
	...
	if notification == nil {
		writeSuccessResponseGateway(c, resolvedProviderKey)
		return
	}
	...
	if err := h.paymentService.HandlePaymentNotification(c.Request().Context(), notification, resolvedProviderKey); err != nil {
		...
		writeGatewayString(c, http.StatusInternalServerError, "handle failed")
		return
	}
...
func writeSuccessResponse(c *gin.Context, providerKey string) {
	writeSuccessResponseGateway(gatewayctx.FromGin(c), providerKey)
}

func writeSuccessResponseGateway(c gatewayctx.GatewayContext, providerKey string) {
	switch providerKey {
	case payment.TypeWxpay:
		c.WriteJSON(http.StatusOK, wxpaySuccessResponse{Code: wxpaySuccessCode, Message: wxpaySuccessMessage})
	case payment.TypeStripe:
		writeGatewayString(c, http.StatusOK, "")
	default:
		writeGatewayString(c, http.StatusOK, "success")
	}
}

func writeGatewayString(c gatewayctx.GatewayContext, status int, body string) {
	if c == nil {
		return
	}
	c.SetHeader("Content-Type", "text/plain; charset=utf-8")
	_, _ = c.WriteBytes(status, []byte(body))
}
