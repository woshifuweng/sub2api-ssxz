...
func (h *OpenAIGatewayHandler) openAISecurityAuditErrorContext(c gatewayctx.GatewayContext, decision *securityaudit.Decision) {
	if decision == nil || c == nil {
		return
	}
	if decision.Legacy != nil && decision.Legacy.Blocked {
		h.errorResponseGateway(c, securityAuditStatus(decision), securityAuditErrorCode(decision), securityAuditMessage(decision))
		return
	}
	errType := "api_error"
	if decision.Kind == securityaudit.DecisionBlock {
	...
	c.WriteJSON(securityAuditStatus(decision), map[string]any{"error": map[string]any{
		"type": errType, "code": securityAuditErrorCode(decision), "message": securityAuditMessage(decision),
	}})
}
...
func (h *GatewayHandler) anthropicSecurityAuditErrorContext(c gatewayctx.GatewayContext, decision *securityaudit.Decision) {
	...
	h.errorResponseGateway(c, securityAuditStatus(decision), errType, securityAuditMessage(decision))
}
