...
func (r *scheduledTestPlanRepository) Create(ctx context.Context, plan *service.ScheduledTestPlan) (*service.ScheduledTestPlan, error) {
	row := r.db.QueryRowContext(ctx, `
		...
		RETURNING
			...
			consecutive_failures, last_failure_reason, paused_at, pause_reason, max_failures_before_pause,
			last_run_at, next_run_at, created_at, updated_at
	`, plan.AccountID, plan.ModelID, plan.CronExpression, plan.Enabled, plan.MaxResults, plan.AutoRecover, plan.ConsecutiveFailures, plan.LastFailureReason, plan.PausedAt, plan.PauseReason, plan.MaxFailuresBeforePause, plan.NextRunAt)
	return scanPlan(row)
}
...
func (r *scheduledTestPlanRepository) Update(ctx context.Context, plan *service.ScheduledTestPlan) (*service.ScheduledTestPlan, error) {
	row := r.db.QueryRowContext(ctx, `
		...
		RETURNING
			...
			consecutive_failures, last_failure_reason, paused_at, pause_reason, max_failures_before_pause,
			last_run_at, next_run_at, created_at, updated_at
	`, plan.ID, plan.ModelID, plan.CronExpression, plan.Enabled, plan.MaxResults, plan.AutoRecover, plan.ConsecutiveFailures, plan.LastFailureReason, plan.PausedAt, plan.PauseReason, plan.MaxFailuresBeforePause, plan.NextRunAt)
	return scanPlan(row)
}
...
func (r *scheduledTestPlanRepository) UpdateAfterRun(ctx context.Context, update service.ScheduledTestRunUpdate) error {
...
func scanPlan(row scannable) (*service.ScheduledTestPlan, error) {
	...
	if err := row.Scan(
		...
		&p.ConsecutiveFailures, &p.LastFailureReason, &p.PausedAt, &p.PauseReason, &p.MaxFailuresBeforePause,
