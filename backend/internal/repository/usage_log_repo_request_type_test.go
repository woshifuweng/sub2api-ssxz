...
func TestUsageLogRepositoryGetStatsWithFiltersRequestID(t *testing.T) {
	...
	mock.ExpectQuery("(?s)SELECT.*FROM usage_logs\\s+WHERE request_id = \\$1").
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"total_account_cost",
			"avg_duration_ms",
		}).AddRow(int64(1), int64(10), int64(20), int64(0), int64(0), int64(0), float64(0.5), float64(0.5), float64(0.5), float64(100)))
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	...
	t.Run("service_tier_is_scanned", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			sql.NullString{},  // session_id
			now,
			sql.NullString{Valid: true, String: "gpt-5.4-billing"}, // billing_model
		}})
		require.NoError(t, err)
		require.NotNil(t, log.ServiceTier)
		require.Equal(t, "priority", *log.ServiceTier)
		require.Equal(t, "gpt-5.4-billing", log.BillingModel)
	})
