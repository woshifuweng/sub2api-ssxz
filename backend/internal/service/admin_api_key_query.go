...
// AdminAPIKeyInventoryService is intentionally separate from AdminService so
// unrelated admin test doubles and feature handlers do not need to implement
// the optional full-site API key inventory.
type AdminAPIKeyInventoryService interface {
	ListAdminAPIKeys(context.Context, AdminAPIKeyListParams) (*AdminAPIKeyListResult, error)
}
