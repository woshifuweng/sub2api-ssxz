...
type antigravityRetryLoopParamsLegacy struct {
	ctx             context.Context
	prefix          string
	account         *Account
	proxyURL        string
...
type antigravityRetryLoopResultLegacy struct {
	resp *http.Response
}

// resolveAntigravityForwardBaseURL 解析转发用 base URL。
// 默认使用 daily（ForwardBaseURLs 的首个地址）；当环境变量为 prod 时使用第二个地址。
func resolveAntigravityForwardBaseURLLegacy() string {
...
type smartRetryActionLegacy int

const (
	smartRetryActionContinueLegacy      smartRetryActionLegacy = iota // 继续默认重试逻辑
	smartRetryActionBreakWithRespLegacy                               // 结束循环并返回 resp
	smartRetryActionContinueURLLegacy                                 // 继续 URL fallback 循环
)

// smartRetryResult 智能重试的结果
type smartRetryResultLegacy struct {
...
func (s *AntigravityGatewayService) handleSmartRetryLegacy(p antigravityRetryLoopParams, resp *http.Response, respBody []byte, baseURL string, urlIdx int, availableURLs []string) *smartRetryResult {
...
func (s *AntigravityGatewayService) handleSingleAccountRetryInPlaceLegacy(
...
func (s *AntigravityGatewayService) antigravityRetryLoopLegacy(p antigravityRetryLoopParams) (*antigravityRetryLoopResult, error) {
	...
	for urlIdx, baseURL := range availableURLs {
		...
		for attempt := 1; attempt <= antigravityMaxRetries; attempt++ {
			...
			if p.c != nil && len(p.body) > 0 {
				setOpsUpstreamRequestBodyAny(p.c, p.body)
			}
...
func shouldRetryAntigravityErrorLegacy(statusCode int) bool {
	switch statusCode {
...
func isURLLevelRateLimitLegacy(body []byte) bool {
...
func isAntigravityConnectionErrorLegacy(err error) bool {
	if err == nil {
		return false
	}
...
func shouldAntigravityFallbackToNextURLLegacy(err error, statusCode int) bool {
...
func getSessionIDLegacy(c *gin.Context) string {
...
// logPrefix 生成统一的日志前缀
func logPrefixLegacy(sessionID, accountName string) string {
	if sessionID != "" {
...
func NewAntigravityGatewayService(
	...
	httpUpstream HTTPUpstream,
	settingService *SettingService,
	internal500Caches ...Internal500CounterCache,
	...
	if len(internal500Caches) > 0 {
		service.internal500Cache = internal500Caches[0]
	}
	return service
}
...
func isModelNotFoundErrorLegacy(statusCode int, body []byte) bool {
	if statusCode != 404 {
		return false
	}
...
func (s *AntigravityGatewayService) ForwardLegacy(ctx context.Context, c *gin.Context, account *Account, body []byte, isStickySession bool) (*ForwardResult, error) {
...
func isSignatureRelatedErrorLegacy(respBody []byte) bool {
	msg := strings.ToLower(strings.TrimSpace(extractAntigravityErrorMessage(respBody)))
	if msg == "" {
...
func isPromptTooLongErrorLegacy(respBody []byte) bool {
	msg := strings.ToLower(strings.TrimSpace(extractAntigravityErrorMessage(respBody)))
	if msg == "" {
		msg = strings.ToLower(string(respBody))
	}
...
func isPassthroughErrorMessageLegacy(msg string) bool {
...
func getPassthroughOrDefaultLegacy(upstreamMsg, defaultMsg string) string {
...
func extractAntigravityErrorMessageLegacy(body []byte) string {
...
func stripThinkingFromClaudeRequestLegacy(req *antigravity.ClaudeRequest) (bool, error) {
	if req == nil {
		return false, nil
	}
...
func stripSignatureSensitiveBlocksFromClaudeRequestLegacy(req *antigravity.ClaudeRequest) (bool, error) {
	if req == nil {
		return false, nil
	}
...
//	          ├─ 成功 → 正常返回
//	          └─ 失败 → 设置模型限流 + 清除粘性绑定 → 切换账号
func (s *AntigravityGatewayService) ForwardGeminiLegacy(ctx context.Context, c *gin.Context, account *Account, originalModel string, action string, stream bool, body []byte, isStickySession bool) (*ForwardResult, error) {
...
func (s *AntigravityGatewayService) shouldFailoverUpstreamErrorLegacy(statusCode int) bool {
	switch statusCode {
...
func isGoogleProjectConfigErrorLegacy(lowerMsg string) bool {
...
const googleConfigErrorCooldownLegacy = 1 * time.Minute

// tempUnscheduleGoogleConfigError 对服务端配置类 400 错误触发临时封禁，
// 避免短时间内反复调度到同一个有问题的账号。
func tempUnscheduleGoogleConfigErrorLegacy(ctx context.Context, repo AccountRepository, accountID int64, logPrefix string) {
...
const emptyResponseCooldownLegacy = 1 * time.Minute

// tempUnscheduleEmptyResponse 对空流式响应触发临时封禁，
// 避免短时间内反复调度到同一个返回空响应的账号。
func tempUnscheduleEmptyResponseLegacy(ctx context.Context, repo AccountRepository, accountID int64, logPrefix string) {
...
func sleepAntigravityBackoffWithContextLegacy(ctx context.Context, attempt int) bool {
...
func isSingleAccountRetryLegacy(ctx context.Context) bool {
...
func setModelRateLimitByModelNameLegacy(ctx context.Context, repo AccountRepository, accountID int64, modelName, prefix string, statusCode int, resetAt time.Time, afterSmartRetry bool) bool {
...
func antigravityFallbackCooldownSecondsLegacy() (time.Duration, bool) {
...
type antigravitySmartRetryInfoLegacy struct {
...
func parseAntigravitySmartRetryInfoLegacy(body []byte) *antigravitySmartRetryInfo {
...
func shouldTriggerAntigravitySmartRetryLegacy(account *Account, respBody []byte) (shouldRetry bool, shouldRateLimitModel bool, waitDuration time.Duration, modelName string, isModelCapacityExhausted bool) {
...
type handleModelRateLimitParamsLegacy struct {
	ctx             context.Context
	prefix          string
	account         *Account
	statusCode      int
...
type handleModelRateLimitResultLegacy struct {
	Handled      bool                           // 是否已处理
...
func (s *AntigravityGatewayService) handleModelRateLimitLegacy(p *handleModelRateLimitParams) *handleModelRateLimitResult {
...
func (s *AntigravityGatewayService) setModelRateLimitAndClearSessionLegacy(p *handleModelRateLimitParams, info *antigravitySmartRetryInfo) {
...
func (s *AntigravityGatewayService) updateAccountModelRateLimitInCacheLegacy(ctx context.Context, account *Account, modelKey string, resetAt time.Time) {
...
func (s *AntigravityGatewayService) handleUpstreamErrorLegacy(
...
func (s *AntigravityGatewayService) getDefaultRateLimitDurationLegacy() time.Duration {
...
func (s *AntigravityGatewayService) resolveResetTimeLegacy(resetAt *int64, defaultDur time.Duration) time.Time {
	if resetAt != nil {
...
type antigravityStreamResultLegacy struct {
	usage            *ClaudeUsage
	firstTokenMs     *int
...
type antigravityClientWriterLegacy struct {
...
func newAntigravityClientWriterLegacy(w gin.ResponseWriter, flusher http.Flusher, prefix string) *antigravityClientWriterLegacy {
	return &antigravityClientWriterLegacy{w: w, flusher: flusher, prefix: prefix}
}

// Write 写入数据到客户端，写入失败时标记断开并返回 false
func (cw *antigravityClientWriterLegacy) WriteLegacy(p []byte) bool {
	if cw.disconnected {
		return false
	}
...
func (cw *antigravityClientWriterLegacy) FprintfLegacy(format string, args ...any) bool {
	if cw.disconnected {
		return false
	}
...
func (cw *antigravityClientWriterLegacy) DisconnectedLegacy() bool { return cw.disconnected }

func (cw *antigravityClientWriterLegacy) markDisconnected() {
...
func handleStreamReadErrorLegacy(err error, clientDisconnected bool, prefix string) (disconnect bool, handled bool) {
...
func (s *AntigravityGatewayService) handleGeminiStreamingResponseLegacy(c *gin.Context, resp *http.Response, startTime time.Time) (*antigravityStreamResult, error) {
...
func (s *AntigravityGatewayService) handleGeminiStreamToNonStreamingLegacy(c *gin.Context, resp *http.Response, startTime time.Time) (*antigravityStreamResult, error) {
...
func getOrCreateGeminiPartsLegacy(response map[string]any) (result map[string]any, existingParts []any, setParts func([]any)) {
	// 深拷贝 response
...
func mergeCollectedPartsToResponseLegacy(response map[string]any, collectedParts []map[string]any) map[string]any {
...
func mergeImagePartsToResponseLegacy(response map[string]any, imageParts []map[string]any) map[string]any {
	if len(imageParts) == 0 {
		return response
	}
...
func mergeTextPartsToResponseLegacy(response map[string]any, textParts []string) map[string]any {
	if len(textParts) == 0 {
		return response
	}
...
func (s *AntigravityGatewayService) writeClaudeErrorLegacy(c *gin.Context, status int, errType, message string) error {
...
func (s *AntigravityGatewayService) WriteMappedClaudeErrorLegacy(c *gin.Context, account *Account, upstreamStatus int, upstreamRequestID string, body []byte) error {
...
func (s *AntigravityGatewayService) writeGoogleErrorLegacy(c *gin.Context, status int, message string) error {
...
func (s *AntigravityGatewayService) handleClaudeStreamToNonStreamingLegacy(c *gin.Context, resp *http.Response, startTime time.Time, originalModel string) (*antigravityStreamResult, error) {
...
func (s *AntigravityGatewayService) handleClaudeStreamingResponseLegacy(c *gin.Context, resp *http.Response, startTime time.Time, originalModel string) (*antigravityStreamResult, error) {
...
func isImageGenerationModelLegacy(model string) bool {
...
func cleanGeminiRequestLegacy(body []byte) ([]byte, error) {
	var payload map[string]any
	if err := json.Unmarshal(body, &payload); err != nil {
		return nil, err
	}
...
func filterEmptyPartsFromGeminiRequestLegacy(body []byte) ([]byte, error) {
	var payload map[string]any
	if err := json.Unmarshal(body, &payload); err != nil {
		return nil, err
	}
...
func (s *AntigravityGatewayService) ForwardUpstreamLegacy(ctx context.Context, c *gin.Context, account *Account, body []byte) (*ForwardResult, error) {
...
func (s *AntigravityGatewayService) streamUpstreamResponseLegacy(c *gin.Context, resp *http.Response, startTime time.Time) *antigravityStreamResult {
...
func (s *AntigravityGatewayService) extractSSEUsageLegacy(line string, usage *ClaudeUsage) {
...
func (s *AntigravityGatewayService) extractClaudeUsageLegacy(body []byte) *ClaudeUsage {
	usage := &ClaudeUsage{}
