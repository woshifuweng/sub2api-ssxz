...
func TestClaudeOpus5_HardcodedFallbackPricing(t *testing.T) {
	...
	// 生产定制：legacy 带日期 opus-4.1 故意 fail-closed（不落 claude-3-opus 兜底价），
	// 避免未知模型被静默错价；上游 165 原测试期望 $15/$75 catch-all，此处固化生产语义。
	_, legacyErr := svc.GetModelPricing("claude-opus-4-1-20250805")
