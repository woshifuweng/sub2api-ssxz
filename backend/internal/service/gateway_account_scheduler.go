...
func (s *defaultGatewayAccountScheduler) selectByLoadBalance(
	...
	if s.service.concurrencyService != nil {
		if batchLoad, loadErr := s.service.concurrencyService.GetAccountsLoadBatch(ctx, loadReq); loadErr == nil {
			...
			if result, ok, legacyErr := s.service.tryAcquireByLegacyOrder(ctx, req.Candidates, req.GroupID, req.SessionHash, req.PreferOAuth); legacyErr != nil {
				return nil, 0, 0, 0, legacyErr
			} else if ok {
