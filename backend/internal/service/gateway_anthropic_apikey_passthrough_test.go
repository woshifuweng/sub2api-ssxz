...
func (u *anthropicHTTPUpstreamRecorder) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, _ *tlsfingerprint.Profile) (*http.Response, error) {
...
func TestGatewayService_AnthropicAPIKeyPassthrough_PublicBaseURLAllowedWhenHostEnforcementDisabled(t *testing.T) {
	...
	req, _, err := svc.buildUpstreamRequestAnthropicAPIKeyPassthrough(context.Background(), c, account, []byte(`{"model":"claude-3-5-sonnet-latest"}`), "k")
	require.NoError(t, err)
