...
func (s *GatewayService) tryAutoReassignFailedProxy(ctx context.Context, accountID int64, failoverErr *UpstreamFailoverError) bool {
	...
	if s.schedulerSnapshot != nil && s.schedulerSnapshot.accountRepo != nil {
...
func (s *GatewayService) applySampledTLSFingerprintProfile(req *http.Request, account *Account) *http.Request {
	...
	if profileID == 0 && s != nil && s.identityService != nil {
		if fp, err := s.identityService.GetSampledFingerprint(req.Context(), account.ID); err == nil && fp != nil {
			profileID, _ = strconv.ParseInt(strings.TrimSpace(fp.TLSProfileID), 10, 64)
		}
	}
	if profileID == 0 {
		return req
...
func (s *GatewayService) TempUnscheduleRetryableError(ctx context.Context, accountID int64, failoverErr *UpstreamFailoverError) {
	...
	// A request-scoped transient error must never touch account health or
	// scheduler state. Check this before any repository lookup because tests
	// and lightweight callers may intentionally provide only the minimal repo
	// methods needed for the non-transient path.
	if failoverErr.RequestScopedTransient {
		return
	}
	if failoverErr.TempUnscheduleFor > 0 && s != nil && s.accountRepo != nil && accountID > 0 {
...
func NewGatewayService(
	...
	if len(extraDependencies) > 0 {
		svc.tlsFPProfileService, _ = extraDependencies[0].(*TLSFingerprintProfileService)
	}
	if len(extraDependencies) > 1 {
		svc.channelService, _ = extraDependencies[1].(*ChannelService)
	}
	if len(extraDependencies) > 2 {
		svc.resolver, _ = extraDependencies[2].(*ModelPricingResolver)
	}
	if len(extraDependencies) > 3 {
		svc.balanceNotifyService, _ = extraDependencies[3].(*BalanceNotifyService)
	}
...
type anthropicCacheControlPayloadLegacy struct {
	Type string `json:"type"`
}

type anthropicSystemTextBlockPayloadLegacy struct {
	Type         string                        `json:"type"`
	Text         string                        `json:"text"`
	CacheControl *anthropicCacheControlPayload `json:"cache_control,omitempty"`
}

type anthropicMetadataPayloadLegacy struct {
...
type claudeOAuthNormalizeOptionsLegacy struct {
	injectMetadata          bool
...
func normalizeClaudeOAuthRequestBodyLegacy(body []byte, modelID string, opts claudeOAuthNormalizeOptions) ([]byte, string) {
	if len(body) == 0 {
		return body, modelID
	}
...
func (s *GatewayService) buildOAuthMetadataUserIDLegacy(parsed *ParsedRequest, account *Account, fp *Fingerprint) string {
...
func (s *GatewayService) SelectAccountWithLoadAwarenessLegacy(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string) (*AccountSelectionResult, error) {
	// 调试日志：记录调度入口参数
...
func (s *GatewayService) selectAccountWithLoadAwarenessSingleGroup(ctx context.Context, group *Group, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string) (*AccountSelectionResult, error) {
	...
	if scheduler == nil {
		if result, ok, err := s.tryAcquireByLegacyOrder(ctx, candidates, groupID, sessionHash, preferOAuth); err != nil {
			return nil, err
		} else if ok {
...
func (s *GatewayService) tryAcquireByLegacyOrderLegacy(ctx context.Context, candidates []*Account, groupID *int64, sessionHash string, preferOAuth bool) (*AccountSelectionResult, bool) {
...
type usageLogWindowStatsBatchProviderLegacy interface {
	GetAccountWindowStatsBatch(ctx context.Context, accountIDs []int64, startTime time.Time) (map[int64]*usagestats.AccountStats, error)
}

type windowCostPrefetchContextKeyTypeLegacy struct{}

var windowCostPrefetchContextKeyLegacy = windowCostPrefetchContextKeyTypeLegacy{}
...
type rpmPrefetchContextKeyTypeLegacy struct{}

var rpmPrefetchContextKeyLegacy = rpmPrefetchContextKeyTypeLegacy{}
...
type selectionFailureStatsLegacy struct {
	Total              int
	Eligible           int
	Excluded           int
	Unschedulable      int
...
type selectionFailureDiagnosisLegacy struct {
	Category string
	Detail   string
}
...
const (
	...
	maxRetryAttemptsLegacy = 5

	// 指数退避：第 N 次失败后的等待 = retryBaseDelay * 2^(N-1)，并且上限为 retryMaxDelay。
	retryBaseDelayLegacy = 300 * time.Millisecond
	retryMaxDelayLegacy  = 3 * time.Second

	// 最大重试耗时（包含请求本身耗时 + 退避等待时间）。
	// 用于防止极端情况下 goroutine 长时间堆积导致资源耗尽。
	maxRetryElapsedLegacy = 10 * time.Second
)
...
func isClaudeCodeClientLegacy(userAgent string, metadataUserID string) bool {
	if metadataUserID == "" {
		return false
	}
...
type cacheControlPathLegacy struct {
	path string
	log  string
}

func collectCacheControlPathsLegacy(body []byte) (invalidThinking []cacheControlPath, messagePaths []string, systemPaths []string) {
...
func enforceCacheControlLimitLegacy(body []byte) []byte {
	if len(body) == 0 {
		return body
	}

	invalidThinking, messagePaths, _, systemPaths := collectCacheControlPaths(body)
	out := body
	modified := false
...
func (s *GatewayService) ForwardContext(ctx context.Context, c gatewayctx.GatewayContext, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	if account.Platform == PlatformAnthropic && c != nil {
		policy := s.evaluateBetaPolicy(ctx, c.HeaderValue("anthropic-beta"), account, parsed.Model)
							...
							if retryReadErr == nil && retryResp.StatusCode == 400 && s.isThinkingBlockSignatureError(retryRespBody) {
								...
								if looksLikeToolSignatureError(msg2) && time.Since(retryStart) < maxRetryElapsed {
									...
									if buildErr2 == nil {
										...
										retryResp2, retryErr2 := s.httpUpstream.DoWithTLS(retryReq2, proxyURL, account.ID, account.Concurrency, s.resolveTLSFingerprintProfile(account))
										if retryErr2 == nil {
											resp = retryResp2
											break
										}
				...
				if isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
					...
					if applied && time.Since(retryStart) < maxRetryElapsed {
						...
						if buildErr == nil {
							...
							budgetRetryResp, retryErr := s.httpUpstream.DoWithTLS(budgetRetryReq, proxyURL, account.ID, account.Concurrency, s.resolveTLSFingerprintProfile(account))
							if retryErr == nil {
...
type anthropicPassthroughForwardInputLegacy struct {
	Body          []byte
	RequestModel  string
...
func (s *GatewayService) buildUpstreamRequestAnthropicAPIKeyPassthroughLegacy(
	ctx context.Context,
	c *gin.Context,
	account *Account,
	body []byte,
...
func (s *GatewayService) forwardBedrockContext(
	...
	bedrockBody, err := PrepareBedrockRequestBodyWithTokens(body, mappedModel, betaTokens, s.isBedrockCCCompatEnabled(ctx, account, parsed.GroupID))
	if err != nil {
...
func (s *GatewayService) buildUpstreamRequestLegacy(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, reqStream bool, mimicClaudeCode bool) (*http.Request, error) {
...
func applyClaudeOAuthHeaderDefaultsLegacy(req *http.Request, isStream bool) {
	if req == nil {
		return
	}
...
type betaBlockedErrorLegacy struct {
	Message string
}
...
type betaPolicyResultLegacy struct {
...
func (s *GatewayService) evaluateBetaPolicyLegacy(ctx context.Context, betaHeader string, account *Account) betaPolicyResult {
...
const betaPolicyFilterSetKeyLegacy = "betaPolicyFilterSet"
...
func (s *GatewayService) getBetaPolicyFilterSetLegacy(ctx context.Context, c *gin.Context, account *Account) map[string]struct{} {
...
func (s *GatewayService) checkBetaPolicyBlockForTokensLegacy(ctx context.Context, tokens []string, account *Account) *BetaBlockedError {
...
var defaultDroppedBetasSetLegacy = buildBetaTokenSet(claude.DroppedBetas)
...
func (s *GatewayService) handleErrorResponseLegacy(ctx context.Context, resp *http.Response, c *gin.Context, account *Account) (*ForwardResult, error) {
...
func (s *GatewayService) handleFailoverSideEffectsLegacy(ctx context.Context, resp *http.Response, account *Account) {
	body, _ := io.ReadAll(io.LimitReader(resp.Body, 2<<20))
...
type streamingResultLegacy struct {
	usage            *ClaudeUsage
	firstTokenMs     *int
	clientDisconnect bool // 客户端是否在流式传输过程中断开
}
...
func (s *GatewayService) handleStreamingResponseContext(ctx context.Context, resp *http.Response, c gatewayctx.GatewayContext, account *Account, startTime time.Time, originalModel, mappedModel string, mimicClaudeCode bool) (*streamingResult, error) {
	...
	sawTerminalEvent := false
	useNoopDeltaKeepalive := shouldUseClaudeCodeNoopDeltaKeepalive(c.HeaderValue("User-Agent"))
	...
	for {
		...
		case <-keepaliveCh:
			...
			if _, werr := c.WriteBytes(0, []byte(keepaliveBlock)); werr != nil {
				clientDisconnected = true
...
type sseUsagePatchLegacy struct {
	inputTokens              int
	hasInputTokens           bool
	outputTokens             int
...
type RecordUsageInputLegacy struct {
...
type APIKeyQuotaUpdaterLegacy interface {
...
type usageLogBestEffortWriterLegacy interface {
	CreateBestEffort(ctx context.Context, log *UsageLog) error
}

// postUsageBillingParams 统一扣费所需的参数
type postUsageBillingParamsLegacy struct {
	Cost                  *CostBreakdown
	User                  *User
	APIKey                *APIKey
	Account               *Account
...
func postUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps) {
	...
	finalizePostUsageBillingLegacy(p, deps)
}
...
func finalizePostUsageBillingLegacy(p *postUsageBillingParams, deps *billingDeps) {
...
type RecordUsageLongContextInputLegacy struct {
	Result                *ForwardResult
	APIKey                *APIKey
	User                  *User
	Account               *Account
...
func (s *GatewayService) buildCountTokensRequestLegacy(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, mimicClaudeCode bool) (*http.Request, error) {
...
func (s *GatewayService) GetAvailableModels(ctx context.Context, groupID *int64, platform string) []string {
	...
	// Anthropic groups may contain OpenAI-compatible transports serving Claude.
	// Other platforms must not inherit models from unrelated accounts that were
	// accidentally attached to the same group.
	if platform != "" && (groupID == nil || platform != PlatformAnthropic) {
