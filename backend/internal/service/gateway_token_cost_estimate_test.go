...
func TestGatewayServiceEstimateGatewayTokenRequestCost(t *testing.T) {
	...
	parsed, err := ParseGatewayRequest(
		NewRequestBodyRef([]byte(`{"model":"claude-sonnet-4","messages":[{"role":"user","content":"hello"}],"max_tokens":200000}`)),
	...
	parsedWithoutLimit, err := ParseGatewayRequest(
		NewRequestBodyRef([]byte(`{"model":"claude-sonnet-4","messages":[{"role":"user","content":"hello"}]}`)),
		domain.PlatformAnthropic,
	)
	require.NoError(t, err)
	...
	geminiParsed, err := ParseGatewayRequest(
		NewRequestBodyRef([]byte(`{"contents":[{"role":"user","parts":[{"text":"hello"}]}],"generationConfig":{"maxOutputTokens":200000}}`)),
