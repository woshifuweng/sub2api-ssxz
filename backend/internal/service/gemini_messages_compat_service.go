	...
	if resp.StatusCode >= 400 {
		...
		if s.rateLimitService != nil {
			...
			case ErrorPolicySkipped:
				...
				if failoverErr := s.skippedErrorPolicyFailoverErrorContext(c, account, resp.StatusCode, respBody, upstreamReqID); failoverErr != nil {
					return nil, failoverErr
				}
				if account.IsCustomErrorCodesEnabled() {
					return nil, s.writeGeminiCustomCodeSkippedErrorContext(c, account, resp.StatusCode, upstreamReqID, respBody, func() {
						_ = s.writeClaudeErrorContext(c, http.StatusInternalServerError, "api_error", geminiCustomCodeSkippedClientMessage)
					})
				}
...
func (s *GeminiMessagesCompatService) forwardNativeWithContext(ctx context.Context, c gatewayctx.GatewayContext, account *Account, originalModel string, action string, stream bool, body []byte) (*ForwardResult, error) {
	...
	if resp.StatusCode >= 400 {
		...
		if s.rateLimitService != nil {
			...
			switch policy {
			case ErrorPolicySkipped:
				if failoverErr := s.skippedErrorPolicyFailoverErrorContext(c, account, resp.StatusCode, respBody, requestID); failoverErr != nil {
					return nil, failoverErr
				}
				if account.IsCustomErrorCodesEnabled() {
					return nil, s.writeGeminiCustomCodeSkippedErrorContext(c, account, resp.StatusCode, requestID, respBody, func() {
						_ = s.writeGoogleErrorContext(c, http.StatusInternalServerError, geminiCustomCodeSkippedClientMessage)
					})
				}
				// 池模式：客户端写出与 ErrorPolicyNone 相同（状态码/响应体保真），仅跳过账号状态标记。
				return nil, s.writeGeminiNativeUpstreamErrorContext(c, account, resp, respBody, requestID, isOAuth)
			case ErrorPolicyMatched, ErrorPolicyTempUnscheduled:
				if policy == ErrorPolicyMatched {
					s.handleGeminiUpstreamError(ctx, account, resp.StatusCode, resp.Header, respBody)
				}
...
func (s *GeminiMessagesCompatService) skippedErrorPolicyFailoverErrorContext(c gatewayctx.GatewayContext, account *Account, statusCode int, respBody []byte, upstreamRequestID string) *UpstreamFailoverError {
	if !s.shouldFailoverGeminiUpstreamError(statusCode) {
		return nil
	}
	upstreamMsg := sanitizeUpstreamErrorMessage(strings.TrimSpace(extractUpstreamErrorMessage(respBody)))
...
func (s *GeminiMessagesCompatService) writeGeminiCustomCodeSkippedErrorContext(c gatewayctx.GatewayContext, account *Account, upstreamStatus int, upstreamRequestID string, body []byte, write func()) error {
	upstreamMsg := sanitizeUpstreamErrorMessage(strings.TrimSpace(extractUpstreamErrorMessage(body)))
	upstreamDetail := s.upstreamErrorDetail(body)
...
func (s *GeminiMessagesCompatService) writeGeminiNativeUpstreamErrorContext(c gatewayctx.GatewayContext, account *Account, resp *http.Response, respBody []byte, requestID string, isOAuth bool) error {
	respBody = unwrapIfNeeded(isOAuth, respBody)
	upstreamMsg := strings.TrimSpace(extractUpstreamErrorMessage(respBody))
	upstreamMsg = sanitizeUpstreamErrorMessage(upstreamMsg)
	upstreamDetail := s.upstreamErrorDetail(respBody)
...
func (s *GeminiMessagesCompatService) writeGeminiMappedErrorContext(c gatewayctx.GatewayContext, account *Account, upstreamStatus int, upstreamRequestID string, body []byte) error {
	...
	c.SetValue(ResponseCommittedKey, true)
	...
	for {
		...
		for _, part := range parts {
			if text, ok := part["text"].(string); ok && text != "" {
				// A Gemini chunk may switch from a function call back to text.
				// Close the tool block first so Anthropic content blocks never overlap.
				if openToolIndex >= 0 {
