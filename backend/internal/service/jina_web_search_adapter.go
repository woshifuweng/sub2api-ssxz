...
func normalizeJinaSearchMaps(items []map[string]any) []jinaSearchHit {
	...
	for _, item := range items {
		urlValue := normalizeSnippet(jinaStringValue(item["url"]), 2048)
		if urlValue == "" {
			urlValue = normalizeSnippet(jinaStringValue(item["link"]), 2048)
		}
		if urlValue == "" {
			continue
		}
		hits = append(hits, jinaSearchHit{
			Title: normalizeSnippet(firstNonEmptyWebSearchString(
				jinaStringValue(item["title"]),
				jinaStringValue(item["name"]),
			), 160),
			URL: urlValue,
			Snippet: normalizeSnippet(firstNonEmptyWebSearchString(
				jinaStringValue(item["snippet"]),
				jinaStringValue(item["description"]),
				jinaStringValue(item["content"]),
				jinaStringValue(item["text"]),
...
func jinaStringValue(value any) string {
