...
func ensureOpenAIResponsesImageGenerationTool(reqBody map[string]any) bool {
	...
	if hasOpenAIImageGenerationTool(reqBody) || hasCodexImageGenerationFunctionTool(reqBody) {
		return false
	}
...
// a Go string or a []any of Chat/Responses text content-part maps.
...
func extractSystemMessagesFromInput(reqBody map[string]any, omitPromotedOption ...bool) bool {
	input, ok := reqBody["input"].([]any)
	if !ok || len(input) == 0 {
		return false
	}
	omitPromoted := len(omitPromotedOption) > 0 && omitPromotedOption[0]
	...
	for _, item := range input {
		...
		if len(omitPromotedOption) > 0 && !omitPromoted {
