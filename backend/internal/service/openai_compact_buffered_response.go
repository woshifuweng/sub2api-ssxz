...
func (s *OpenAIGatewayService) readOpenAICompactBufferedResponseContext(
	...
	scanErr := scanner.Err()
	if finalBody != nil {
		bodyText := rawBody.String()
		if len(gjson.GetBytes(finalBody, "output").Array()) == 0 {
			if outputJSON, reconstructed := reconstructResponseOutputFromSSE(bodyText); reconstructed {
				if patched, patchErr := sjson.SetRawBytes(finalBody, "output", outputJSON); patchErr == nil {
					finalBody = patched
				}
			}
		} else if !responsesOutputHasCompactionItem(finalBody) {
			if item, found := findRawCompactionItemFromSSE(bodyText); found {
				if patched, patchErr := sjson.SetRawBytes(finalBody, "output.-1", item); patchErr == nil {
					finalBody = patched
				}
			}
		}
