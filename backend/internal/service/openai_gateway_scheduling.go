...
func (s *OpenAIGatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*AccountSelectionResult, error) {
	...
	var waitingSelection *AccountSelectionResult
	for _, entry := range chain {
		selection, selectErr := s.selectAccountWithLoadAwareness(ctx, &entry.GroupID, PlatformOpenAI, sessionHash, requestedModel, excludedIDs, false, "", true)
		if selectErr == nil {
			if selection != nil && selection.WaitPlan != nil {
				if waitingSelection == nil {
					waitingSelection = selection
				}
				continue
			}
			return selection, nil
		}
		if !errors.Is(selectErr, ErrNoAvailableAccounts) && !errors.Is(selectErr, ErrNoAvailableCompactAccounts) {
			return nil, selectErr
		}
	}
	if waitingSelection != nil {
		return waitingSelection, nil
	}
