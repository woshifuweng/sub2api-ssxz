	...
	if result != nil && result.AudioUsage != nil {
		if resolved := s.resolveOpenAIChannelPricing(ctx, billingModel, apiKey); resolved != nil &&
			...
			return s.billingService.CalculateCostUnified(CostInput{
				...
				RateMultiplier: webSearchMultiplier, Resolver: s.modelPricingResolver, Resolved: resolved,
			})
		}
...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageTokenCost(
	...
	if s.modelPricingResolver != nil && apiKey.Group != nil {
		...
		return s.billingService.CalculateCostUnified(CostInput{
			...
			ServiceTier: serviceTier, Resolver: s.modelPricingResolver,
...
func (s *OpenAIGatewayService) calculateOpenAIImageCost(
	...
	if resolved != nil && resolved.Source == PricingSourceGroup &&
		...
		cost, err := s.billingService.CalculateCostUnified(CostInput{
			...
			RateMultiplier: multiplier, Resolver: s.modelPricingResolver, Resolved: resolved,
		})
		if err == nil {
			return cost
		}
...
func (s *OpenAIGatewayService) resolveOpenAIChannelPricing(ctx context.Context, billingModel string, apiKey *APIKey) *ResolvedPricing {
	...
	gid := apiKey.Group.ID
	resolved := s.modelPricingResolver.Resolve(ctx, PricingInput{Model: billingModel, GroupID: &gid, Group: apiKey.Group})
