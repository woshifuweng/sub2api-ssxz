...
func (s *OpenAIGatewayService) ForwardImagesContext(
	...
	// Official OpenAI OAuth accounts serve the Images API through the Responses
	// image-generation bridge rather than the native Images endpoint. Preserve
	// that production route before applying the native/chat-web capability gate.
	if account != nil && account.IsOpenAIOAuth() && !account.IsOpenAIChatWebMode() {
		if c == nil {
			return nil, fmt.Errorf("OpenAI OAuth images require a gateway context")
		}
		ginContext, ok := c.Native().(*gin.Context)
		if !ok || ginContext == nil {
			return nil, fmt.Errorf("OpenAI OAuth images require a Gin gateway context")
		}
		return s.forwardOpenAIImagesOAuth(ctx, ginContext, account, parsed, channelMappedModel)
	}
...
func (s *OpenAIGatewayService) forwardOpenAIImagesAPIKeyContext(
	...
	upstreamSize := account.GetMappedOpenAIImageSize(parsed.Size)
	if upstreamSize != "" && !strings.EqualFold(upstreamSize, parsed.Size) {
		forwardBody, forwardContentType, err = rewriteOpenAIImagesSize(forwardBody, forwardContentType, upstreamSize)
		if err != nil {
			return nil, err
		}
	}
	...
	upstreamReq, err := s.buildOpenAIImagesRequestContext(upstreamCtx, c, account, forwardBody, forwardContentType, token, parsed.Endpoint)
	if err != nil {
		return nil, err
	}
...
func rewriteOpenAIImagesModel(body []byte, contentType string, model string) ([]byte, string, error) {
	return rewriteOpenAIImagesField(body, contentType, "model", model)
}

func rewriteOpenAIImagesSize(body []byte, contentType string, size string) ([]byte, string, error) {
	return rewriteOpenAIImagesField(body, contentType, "size", size)
}

func rewriteOpenAIImagesField(body []byte, contentType string, field string, value string) ([]byte, string, error) {
	...
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err == nil && strings.EqualFold(mediaType, "multipart/form-data") {
		return rewriteOpenAIImagesMultipartField(body, contentType, field, value)
	}
	rewritten, err := sjson.SetBytes(body, field, value)
	if err != nil {
...
func rewriteOpenAIImagesMultipartField(body []byte, contentType string, field string, value string) ([]byte, string, error) {
	_, params, err := mime.ParseMediaType(contentType)
	if err != nil {
	...
	for {
		...
		if formName == field && part.FileName() == "" {
			if _, err := target.Write([]byte(value)); err != nil {
				_ = part.Close()
				return nil, "", fmt.Errorf("rewrite multipart %s: %w", field, err)
			}
			fieldWritten = true
			_ = part.Close()
			continue
...
func (s *OpenAIGatewayService) handleOpenAIImagesStreamingResponseContext(
	...
	processLine := func(line []byte) {
		...
		if !clientDisconnected {
			...
			} else if flushErr := c.Flush(); flushErr != nil {
				clientDisconnected = true
				logger.LegacyPrintf("service.openai_gateway", "[OpenAI] Images stream flush failed, continue draining upstream for billing")
			} else {
				lastDownstreamWriteAt = time.Now()
			}
		}
	...
	for {
		...
		case <-intervalCh:
			...
			if !clientDisconnected {
				payload := append([]byte("event: error\ndata: "), buildOpenAIImagesStreamErrorBody(fmt.Sprintf("upstream image stream idle for %s", streamInterval))...)
				payload = append(payload, []byte("\n\n")...)
				_, _ = c.WriteBytes(0, payload)
				_ = c.Flush()
			}
		...
		case <-keepaliveCh:
			...
			if _, writeErr := c.WriteBytes(0, []byte(":\n\n")); writeErr != nil {
				clientDisconnected = true
				continue
			}
