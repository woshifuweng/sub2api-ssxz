...
func (u *httpUpstreamRecorder) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, _ *tlsfingerprint.Profile) (*http.Response, error) {
...
func (u *sequenceHTTPUpstreamRecorder) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, _ *tlsfingerprint.Profile) (*http.Response, error) {
	return u.Do(req, proxyURL, accountID, accountConcurrency)
}
...
func TestOpenAIGatewayService_CodexCLIOnly_AllowOfficialClientFamilies(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "originator_codex_chatgpt_desktop", ua: "codex_chatgpt_desktop/1.0.0", originator: "codex_chatgpt_desktop"},
	}
...
func TestOpenAIGatewayService_APIKeyPassthrough_PreservesBodyAndUsesResponsesEndpoint(t *testing.T) {
	...
	c.Request.Header.Set("User-Agent", "curl/8.0")
	c.Request.Header.Set("X-Test", "keep")
	c.Request.Header.Set("Authorization", "Bearer sk-ssxz-customer-key-must-not-forward")
	c.Request.Header.Set("X-Api-Key", "sk-ssxz-customer-key-must-not-forward")
	...
	require.NotContains(t, upstream.lastReq.Header.Get("Authorization"), "sk-ssxz-customer-key")
	require.Empty(t, upstream.lastReq.Header.Get("X-Api-Key"))
	require.Equal(t, "curl/8.0", upstream.lastReq.Header.Get("User-Agent"))
	require.Empty(t, upstream.lastReq.Header.Get("X-Test"))
}
