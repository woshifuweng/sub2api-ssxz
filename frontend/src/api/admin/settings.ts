...
const WECHAT_CONNECT_MODE_OPTIONS: WeChatConnectModeOption[] = [
  ...
  { value: 'mp', labelZh: '公众号', labelEn: 'Official Account' },
  { value: 'mobile', labelZh: '移动应用', labelEn: 'Mobile App' }
]
...
export function buildAuthSourceDefaultsState(
  ...
  return AUTH_SOURCE_TYPES.reduce((result, source) => {
...
export async function getEmailTemplates(): Promise<EmailTemplateListResponse> {
  const { data } = await apiClient.get<EmailTemplateListResponse>('/admin/settings/email-templates')
  return data
}
...
export async function restoreOfficialEmailTemplate(event: string, locale: string): Promise<EmailTemplateDetail> {
