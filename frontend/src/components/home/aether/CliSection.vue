<template>
  <section
    ref="sectionRef"
    class="relative z-30 min-h-screen snap-start flex items-center justify-center px-4 sm:px-8 md:px-16 lg:px-20 py-20"
  >
    <div class="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center w-full">
      ...
      >
        ...
        <!-- Title -->
        <h2
          class="text-4xl md:text-5xl font-bold text-[hsl(var(--foreground))] mb-6 transition-all duration-700"
          :style="titleStyle"
        >
          {{ title }}
        </h2>
        ...
        <!-- Description -->
        <p
          class="text-lg text-[hsl(var(--muted-foreground))] mb-4 transition-all duration-700"
          :style="descStyle"
        >
          {{ description }}
        </p>
...
import type { PlatformOption } from './platform-presets'
