...
const createStar = (id: number, sizeRange: [number, number, number, number, number, number], opacityBase: number, opacityRange: number, zIndexBase: number): Star => {
...
const createStarLayers = () => {
  const far: Star[] = []
  for (let i = 0; i < 30; i++) {
    far.push(createStar(i, [6, 4, 10, 6, 14, 6], 0.2, 0.25, 0))
  }
  farStars.value = far

  const mid: Star[] = []
  for (let i = 0; i < 18; i++) {
    mid.push(createStar(i, [18, 8, 24, 10, 32, 10], 0.35, 0.35, 30))
  }
  midStars.value = mid

  const near: Star[] = []
  for (let i = 0; i < 10; i++) {
    near.push(createStar(i, [40, 15, 55, 20, 70, 25], 0.5, 0.5, 100))
  }
