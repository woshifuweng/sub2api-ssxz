<template>
  <Transition name="logo-fade">
    <!-- Aether native provider artwork and timing, preserved for SSXZ. -->
    <svg
...
type LogoType = 'claude' | 'openai' | 'gemini'
...
const geminiFillRadius = ref(15)
const animationKey = ref(0)
const drawDuration = 1000
const openaiOutlineDuration = 1000
...
// Start the original provider-specific animation sequence.
const startAnimation = () => {
  ...
  addTimer(() => {
    if (props.type === 'gemini') {
      addTimer(animateFill, geminiOutlineDuration)
    } else if (props.type === 'openai') {
      addTimer(() => { drawComplete.value = true }, openaiOutlineDuration)
    } else {
      addTimer(() => { drawComplete.value = true }, drawDuration)
    }
  }, props.animDelay)
}
...
watch(
  ...
  async (_newType, oldType) => {
    ...
    if (oldType !== undefined) animationKey.value++
    await nextTick()
...
defineExpose({ fillComplete, drawComplete })
...
const pathLength = computed(() => {
  const lengths: Record<LogoType, number> = { claude: 300, openai: 200, gemini: 150 }
  return lengths[props.type]
})

const pathData = computed(() => {
  const paths: Record<LogoType, string> = {
    claude:
...
const strokeColor = computed(() => {
  if (props.type === 'openai') return 'currentColor'
  if (props.type === 'claude') return '#D97757'
  return 'url(#' + pathId.value + '-gradient)'
})

const transformOrigin = computed(() =>
  props.type === 'claude' ? '12.6px 12.7px' : '12px 12px'
)
</script>
