...
const { isAgent: isResellerAgent, isManager: isResellerManager } = useResellerNavRole()
...
function isActive(path: string) {
  ...
  // 经销商中心有 4 个子页面(withdrawals / recruits / commission / invite),
  // 它们没有各自的菜单项。精确匹配会导致停在子页面时整个侧栏零高亮,
  // 所以让父项在子路由下保持高亮。/app/reseller/manager 例外——它自己是一项。
  if (normalizedPath === '/app/reseller') {
    return route.path === '/app/reseller'
      || (route.path.startsWith('/app/reseller/') && route.path !== '/app/reseller/manager')
  }
