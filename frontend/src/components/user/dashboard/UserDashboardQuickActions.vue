<template>
  <div class="card">
    ...
    <div class="space-y-3 p-4">
      <button @click="router.push('/app/keys')" class="group flex w-full items-center gap-4 rounded-xl bg-gray-50 p-4 text-left transition-all duration-200 hover:bg-gray-100 dark:bg-dark-800/50 dark:hover:bg-dark-800">
        ...
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-gray-900 dark:text-white">API 密钥</p>
          <p class="text-xs text-gray-500 dark:text-dark-400">创建 Key 并接入第三方客户端</p>
        </div>
        <Icon
          name="chevronRight"
          size="md"
      ...
      <button @click="router.push('/app/usage')" class="group flex w-full items-center gap-4 rounded-xl bg-gray-50 p-4 text-left transition-all duration-200 hover:bg-gray-100 dark:bg-dark-800/50 dark:hover:bg-dark-800">
      ...
      <button @click="router.push(paymentEnabled ? '/app/purchase' : '/app/orders')" class="group flex w-full items-center gap-4 rounded-xl bg-gray-50 p-4 text-left transition-all duration-200 hover:bg-gray-100 dark:bg-dark-800/50 dark:hover:bg-dark-800">
        ...
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ paymentEnabled ? '充值 / 订阅' : '我的订单' }}</p>
          <p class="text-xs text-gray-500 dark:text-dark-400">{{ paymentEnabled ? '充值或查看订单记录' : '查看订单记录和到账状态' }}</p>
        </div>
        <Icon
          name="chevronRight"
          size="md"
