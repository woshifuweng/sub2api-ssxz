...
describe('F0FoundationPreview', () => {
  it('renders the approved component families and switches the scoped theme', async () => {
    ...
    expect(wrapper.findAll('.f0-card').length).toBeGreaterThanOrEqual(7)
  ...
  it('remains absent from production routing and global style entry points', () => {
    ...
    expect(foundationStyleSource).toContain('--primary: 220 10% 10%;')
    expect(foundationStyleSource).toContain('--brand-accent: 210 14% 38%;')
    ...
    expect(darkThemeBlock).toContain('--primary: 0 0% 96%;')
    expect(darkThemeBlock).toContain('--brand-accent: 210 15% 68%;')
