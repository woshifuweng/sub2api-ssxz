...
const affiliateKeys = [
  'affiliate.rateClosedValue',
  'affiliate.notOpenTitle',
  'affiliate.notOpenBody',
  'affiliate.transfer.button',
  'affiliate.transfer.transferring',
  'affiliate.noRecordsTitle',
] as const
...
describe.each(['zh', 'en'] as const)('runtime locale loader (%s)', (locale) => {
  ...
  it('loads every referral workbench message as display text', async () => {
    await loadLocaleMessages(locale)
    const messages = i18n.global.getLocaleMessage(locale) as Record<string, any>
