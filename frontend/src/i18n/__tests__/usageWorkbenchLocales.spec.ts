...
describe('usage workbench locale copy', () => {
  it('explains usage, no-charge, and customer-facing charge results in zh', () => {
    const copy = [
    ...
    expect(copy).toContain('实际扣费')
    expect(copy).toContain('暂时无法加载')
    expect(copy).toContain('不会展示成空趋势')
    expect(copy).toContain('不会自动跳到旧版页面')
  })

  it('explains usage, no-charge, and customer-facing charge results in en', () => {
    const copy = [
    ...
    expect(copy).toContain('Actual charge {actual}')
    expect(copy).toContain('temporarily unavailable')
    expect(copy).toContain('unavailable trends are not presented as empty data')
