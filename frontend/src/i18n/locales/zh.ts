    ...
    useKeyModal: {
      ...
      thirdParty: {
        ...
        quickStartDescription:
          '大多数客户端只需要这四项。如果客户端同时有“官网链接”和“API 请求地址”，官网链接填本站域名，API 请求地址填这里的 Base URL。',
        quickStartBaseUrl: 'Base URL',
        quickStartApiKey: 'API Key',
        quickStartApiKeyHint: '使用创建后只显示一次的完整 Key。列表里的脱敏值不能连接。',
        quickStartModel: '模型',
        quickStartModelHint: '用客户端下拉列表，或刷新模型列表。可用模型以当前账号套餐为准。',
        quickStartSpeed: '想更快',
        quickStartSpeedHint: '选择轻量模型、降低推理强度，或减少一次性输入内容。',
        ccSwitchQuickTitle: 'CC Switch 填写方式',
        ccSwitchQuickDescription: '通过 CC Switch 连接 Codex 时，优先按这里填写。',
        ...
        ccSwitchQuickModel: '模型：从可用列表里选 GPT-5.5、GPT-5.4 或 GPT-5.4-Mini。',
        ...
        connectionChecklistModels: '客户端模型列表为空时，刷新客户端模型列表，或打开 /v1/models 检查可用模型。',
        connectionChecklistBalance: '如果请求失败，检查 Key 状态、账户余额、额度和使用记录。',
        connectionChecklistRestart: '修改 CC Switch / Codex 配置后，请完全退出客户端再重新打开；只刷新页面可能仍读取旧配置。',
        troubleshootingTitle: '常见错误排查',
        troubleshooting401:
          '401 通常是 API Key 填错、不完整、复制了脱敏值，或填到了错误字段。',
        troubleshooting403:
          '403 通常是余额、额度、Key 状态或所选模型当前不可用。',
        troubleshooting429:
          '429 通常是请求太频繁。稍等一会儿，降低并发，或选择更轻量的模型。',
        troubleshooting503:
          '503 通常是当前线路繁忙或暂时不可用。稍后重试，或换一个可用模型；修改配置后请完全重启客户端。',
