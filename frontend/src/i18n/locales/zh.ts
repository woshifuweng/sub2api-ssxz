export default {
  ...
  nav: {
    ...
    paymentDashboard: '收款概览',
    orderManagement: '充值订单',
    paymentPlans: '套餐设置',
    myAccount: '我的账户',
    lightMode: '浅色模式',
    darkMode: '深色模式',
  ...
  affiliate: {
    ...
    eyebrow: '账户运营',
    loadFailedTitle: '邀请返利暂时无法加载',
    loadFailedBody: '刷新后会重新获取推广码和邀请记录。已有记录会继续保留。',
    reload: '重新加载',
    currentRate: '当前活动比例',
    rateHint: '实际奖励以系统记录为准',
    inviteCount: '邀请人数',
    inviteCountHint: '通过专属链接注册的人数',
    availableQuota: '可结算额度',
    availableQuotaHint: '可转入账户余额',
    totalRewards: '累计奖励',
    pending: '待确认：{amount}',
    exclusiveInvite: '专属邀请',
    inviteDescription: '复制专属链接发给客户，对方通过链接注册后会自动计入邀请记录。有效使用后，奖励会按活动规则进入可结算额度。',
    code: '推广码',
    link: '推广链接',
    copy: '复制',
    codeCopied: '推广码已复制',
    linkCopied: '推广链接已复制',
    settleTitle: '结算到余额',
    settleDescription: '将可结算额度转入账户余额，到账后可直接用于站内调用。待确认额度会在满足规则后再进入可结算额度。',
    settling: '结算中...',
    transfer: '转入余额',
    recordsTitle: '邀请记录',
    recordsDescription: '这里显示通过你推广链接注册的客户，以及已计入的累计奖励。具体到账以系统记录为准。',
    noRecords: '暂无邀请记录。',
    user: '用户',
    name: '名称',
    creditedReward: '已计奖励',
  ...
  auth: {
    ...
    portalDescription: '使用同一套账号与密钥，稳定接入你日常使用的模型服务。',
    createAccount: '创建账户',
  ...
  keys: {
    ...
    workbenchGuide: {
      ...
      description:
        'API Key 用于 Cherry Studio、Chatbox、CC Switch 等第三方客户端。给客户测试时优先提供 Base URL、完整 Key 和推荐模型。',
      baseUrlLabel: 'Base URL',
      copyBaseUrl: '复制 Base URL',
      baseUrlCopied: 'Base URL 已复制',
      baseUrlRootLabel: 'Base URL（Claude Code / Codex CLI）',
      copyBaseUrlRoot: '复制 Base URL（Claude Code / Codex CLI）',
      baseUrlRootCopied: 'Base URL（无 /v1）已复制',
    ...
    enableKeyTitle: '启用 API Key',
    disableKeyTitle: '禁用 API Key',
    enableConfirmMessage: '确认启用 API Key “{name}” 吗？启用后，使用这个 Key 的第三方客户端可以继续调用已开放的模型。',
    disableConfirmMessage: '确认禁用 API Key “{name}” 吗？禁用后，使用这个 Key 的第三方客户端会停止工作，直到重新启用。',
    apiKey: 'API 密钥',
    group: '分组',
    noGroup: '无分组',
    searchGroup: '搜索分组...',
    ...
    useKeyModal: {
      ...
      thirdParty: {
        ...
        title: '第三方客户端接入',
        description:
          '在客户端里新增 OpenAI-compatible 或自定义 OpenAI 服务，然后按下方字段填写。不了解高级设置时，不要随意改其它参数。',
        quickStartTitle: '先填这几项',
        quickStartDescription:
          '大多数客户端只需要这几项。如果客户端同时有“官网链接”和“API 请求地址”，官网链接填本站域名，API 请求地址填这里的 Base URL。',
        ...
        ccSwitchQuickDescription: '通过 CC Switch 连接时，优先使用一键导入；客户版本不兼容时，再按这里手动填写。',
        ...
        modelLabel: '模型',
        modelHint: '使用当前账号套餐可用的模型。',
        modelsEndpointLabel: '模型列表验证',
        connectionChecklistTitle: '连接前检查',
        connectionChecklistBaseUrl: 'Base URL 必须填写本站地址加 /v1，不要填其它服务商地址。',
        connectionChecklistEndpoint: '如果客户端同时有“官网链接”和“API 请求地址”，官网链接填本站域名，API 请求地址必须填上方 Base URL。',
        connectionChecklistFullKey: 'API Key 必须使用创建弹窗里保存的完整值，列表中的脱敏值不能用于连接。',
        connectionChecklistModels: '客户端模型列表为空时，刷新客户端模型列表，或打开 /v1/models 检查当前 Key 可用模型。',
        connectionChecklistBalance: '如果请求失败，先检查 Key 状态、账户余额、额度、模型分组和使用记录。',
        connectionChecklistRestart: '修改 CC Switch / Codex 配置后，请完全退出客户端再重新打开；只刷新页面可能仍读取旧配置。',
        troubleshootingTitle: '常见错误排查',
        troubleshooting401:
          '401 通常是 API Key 填错、不完整、复制了脱敏值，或填到了错误字段；重新复制完整 Key 再试。',
        troubleshooting403:
          '403 通常是余额、额度、Key 状态、分组权限或所选模型当前不可用；换推荐模型或调整额度后再试。',
        troubleshooting429:
    ...
    resetQuotaConfirmMessage: '要将 "{name}" 的 API Key 自限额计数（${used}）重置为 0 吗？这不会删除历史用量、不会改账单、也不会返还余额。',
    ...
    resetRateLimitConfirmMessage: '要重置 "{name}" 的 API Key 速率限制计数吗？这只清空时间窗口计数，不会删除历史用量、不会改账单、也不会返还余额。',
  ...
  usage: {
    ...
    workbench: {
      title: '用量信息',
      subtitle: '查看账户余额、本月消耗和最近使用明细。没有用量记录时只显示空状态，不跳转到旧版用量页。',
      eyebrow: '账户计量',
      balanceTitle: '账户余额',
      ...
      monthlyUsageSummary: '本月 {requests} 次请求，累计用量 {tokens}。',
      statsLoadError: '本月统计暂时无法加载，请刷新重试。',
      monthlyUsageTitle: '每日用量趋势',
      monthlyUsageDescription: '使用真实日粒度记录，可按周期查看消费、请求和 Token 变化。',
      realDataBadge: '真实数据',
      demoDataBadge: '样例数据',
      monthlyChartLabel: '每日用量趋势图表',
      dailyCostTrend: '每日实际消费',
      dailyRequestTrend: '每日请求数',
      dailyTokenTrend: '每日 Token 用量',
      actualChargeTrendHint: '只统计最终实际扣除金额',
      requestTrendHint: '按成功记录汇总每日请求',
      tokenTrendHint: '输入、输出与缓存 Token 合计',
      period7Days: '近 7 天',
      period30Days: '近 30 天',
      period90Days: '近 90 天',
      ...
      average: '均值',
      peak: '峰值',
      noMonthlyUsageTitle: '暂无用量趋势数据',
      noMonthlyUsageDescription: '后续产生聊天、图片生成或第三方客户端调用后，这里会展示趋势。',
      trendLoadError: '用量趋势暂时无法加载',
      trendLoadErrorHint: '明细和余额仍可单独刷新；趋势加载失败时不会展示成空趋势。',
      usageDetailsTitle: '用量明细',
      usageDetailsDescription: '展示最近调用记录，方便查看模型、用量和扣费结果。成功调用按实际用量扣费，失败或未完成的请求不扣费。点击任意一行可查看第三方接入来源、定位码和计费明细。',
      refresh: '刷新',
      loading: '正在加载用量',
      detailsLoadError: '用量明细暂时无法加载',
      detailsLoadErrorHint: '可以稍后刷新，不会自动跳到旧版页面。',
      noDetailsTitle: '暂无用量明细',
      noDetailsDescription: '真实调用产生后会在这里显示。',
      createdAt: '创建时间',
      kind: '类型',
      endpoint: 'Endpoint',
      model: '模型',
      ...
      billingBasis: '计费方式',
      billingBalance: '余额扣费',
      billingSubscription: '订阅额度',
      billingNoCharge: '未扣余额',
      standardVsActual: '实际扣费 {actual}',
      actualChargeBasis: '实际扣费 {amount}',
      noChargeBasis: '本条记录未扣除余额。',
      performance: '处理耗时',
      performanceHealthy: '正常',
      performanceUnknown: '暂无记录',
      performanceSlowFirstToken: '启动稍慢',
      performanceSlowTotal: '处理较久',
      performanceNoRecord: '暂无可展示耗时',
      performanceSummary: '开始响应 {firstToken}；总耗时 {duration}',
      performanceSlowFirstTokenHint: '本次任务启动稍慢，通常和模型档位、任务复杂度或外部步骤有关。想更快可换轻量模型或降低推理强度。',
      performanceSlowTotalHint: '本次任务处理时间较长，通常表示查询更深入或步骤更多。想更快可换轻量模型、降低推理强度，或缩短一次性输入。',
      supportCode: '定位码',
      copySupportCode: '复制定位码',
      supportCodeCopied: '定位码已复制',
      copy: '复制',
      copied: '已复制',
      duration: '耗时',
      fee: '扣费',
      feeTooltip: '成功完成的调用才会扣费；费用为 $0.0000 表示本条未扣除余额。失败或未完成的请求不产生扣费。',
      expandRow: '展开详情',
      collapseRow: '收起详情',
      detailNote: '备注',
      tokenBreakdown: '输入 {input} · 输出 {output} · 缓存写入 {cacheWrite} · 缓存读取 {cacheRead}',
      noCharge: '未扣费',
      zeroTokenCharged: '图片/固定计费项目',
      usageKindImage: '图片生成',
      usageKindChat: '对话',
      usageKindThirdParty: '第三方接入',
      usageKindWeb: '网页端',
      imageAmount: '{count} 张',
  ...
  payment: {
    ...
    currentBalance: '当前余额',
    notAvailable: '当前暂未开放在线充值/订阅入口，请先使用已有额度或按页面提示的方式充值。',
    quickAmounts: '快捷金额',
    customAmount: '自定义金额',
    enterAmount: '请输入充值金额',
    amountNoMethod: '请选择支付方式',
    amountTooLow: '充值金额不能低于 {min}',
    amountTooHigh: '充值金额不能高于 {max}',
    paymentMethod: '支付方式',
    paymentAmount: '支付金额',
    fee: '手续费',
    actualPay: '实际支付',
    creditedBalance: '到账余额',
    rechargeRatePreview: '本次充值预计到账 ${usd}',
    createOrder: '创建订单',
    amountLabel: '金额',
    selectPlan: '选择套餐',
    noPlans: '暂无可购买套餐',
    ...
    groupFallback: '分组 {id}',
    refundReason: '退款原因',
    refundReasonPlaceholder: '请填写退款原因',
    confirmCancel: '确认取消',
    stripePay: 'Stripe 支付',
    stripeMissingParams: '缺少 Stripe 支付参数',
    ...
    modePopup: '弹窗支付',
    supportedTypes: '支持类型',
    enableTypesFirst: '请先启用至少一种支付类型',
    allowUserRefund: '允许用户申请退款',
    refundEnabled: '已开启退款',
    limitsTitle: '支付限额',
    limitsHint: '不填写则使用全局限额。',
    limitsUseGlobal: '使用全局限额',
    limitsNoLimit: '不限额',
    limitSingleMin: '单笔最低',
    limitSingleMax: '单笔最高',
    limitDaily: '每日限额',
    paymentGuideTrigger: '支付引导',
    guideCallLabel: '调用说明',
    guideOpenLabel: '打开说明',
    guideFallbackLabel: '兜底说明',
    validationNameRequired: '请填写渠道名称',
    validationFieldRequired: '请填写必填配置',
    field_notifyUrl: '回调地址',
    field_returnUrl: '返回地址',
    field_certSerial: '证书序列号',
    field_: '配置字段',
    stripeWebhookHint: '请在 Stripe 控制台配置 Webhook 回调地址。',
    providerAlipay: '支付宝',
    providerWxpay: '微信支付',
    providerStripe: 'Stripe',
    providerEasypay: '易支付',
    typeDisabled: '未启用',
    alipayGuideSummary: '根据支付模式选择对应的支付宝接入方式。',
    alipayGuideFaceToFaceTitle: '当面付',
    alipayGuideFaceToFaceCall: '适合扫码支付场景。',
    alipayGuideFaceToFaceOpen: '打开支付宝扫码完成支付。',
    alipayGuideFaceToFaceFallback: '如无法唤起，请使用二维码支付。',
    alipayGuidePagePayTitle: '电脑网站支付',
    alipayGuidePagePayCall: '适合桌面浏览器跳转支付。',
    alipayGuidePagePayOpen: '打开支付宝收银台。',
    alipayGuidePagePayFallback: '如无法跳转，请返回重新下单。',
    alipayGuideWapTitle: '手机网站支付',
    alipayGuideWapCall: '适合移动浏览器支付。',
    alipayGuideWapOpen: '打开支付宝移动支付页。',
    alipayGuideWapFallback: '如无法打开，请复制链接到浏览器。',
    wxpayGuideSummary: '根据支付模式选择对应的微信支付接入方式。',
    wxpayGuideNote: '微信支付需要按商户平台要求配置回调和授权目录。',
    wxpayGuideNativeTitle: 'Native 扫码支付',
    wxpayGuideNativeCall: '适合桌面端扫码支付。',
    wxpayGuideNativeOpen: '使用微信扫码完成支付。',
    wxpayGuideNativeFallback: '如二维码不可用，请重新创建订单。',
    wxpayGuideH5Title: 'H5 支付',
    wxpayGuideH5Call: '适合移动浏览器支付。',
    wxpayGuideH5Open: '打开微信 H5 支付页。',
    wxpayGuideH5Fallback: '如无法打开，请切换浏览器重试。',
    wxpayGuideJsapiTitle: 'JSAPI 支付',
    wxpayGuideJsapiCall: '适合微信内支付。',
    wxpayGuideJsapiOpen: '在微信内完成支付。',
    wxpayGuideJsapiFallback: '请在微信内打开页面后重试。',
    methods: {
      alipay: '支付宝',
      wxpay: '微信支付',
      stripe: 'Stripe',
    ...
    qr: {
      scanToPay: '扫码支付',
      scanAlipay: '支付宝扫码支付',
      scanAlipayHint: '请使用支付宝扫码完成支付',
      scanWxpay: '微信扫码支付',
      scanWxpayHint: '请使用微信扫码完成支付',
      expiresIn: '二维码剩余时间',
      ...
      cancelled: '订单已取消',
      cancelledDesc: '该订单已取消，不会扣除余额。',
      expired: '二维码已过期',
      expiredDesc: '请重新创建订单。',
      openPayWindow: '打开支付页面',
      payInNewWindow: '在新窗口支付',
    ...
    errors: {
      ...
      cancelRateLimited: '操作过于频繁，请稍后再试。',
      mobilePaymentFallbackToQr: '移动支付暂不可用，已切换为扫码支付。',
      alipayDesktopQrHint: '桌面端请使用二维码完成支付宝支付。',
      alipayDesktopUnavailable: '当前支付宝桌面支付不可用。',
      alipayMobileOpenHint: '请在移动浏览器中打开支付宝支付页。',
      alipayMobileUnavailable: '当前支付宝移动支付不可用。',
      wechatH5NotAuthorized: '微信 H5 支付未授权。',
      wechatJsapiFailed: '微信 JSAPI 支付启动失败。',
      wechatJsapiUnavailable: '当前微信 JSAPI 支付不可用。',
      wechatOpenInWeChatHint: '请在微信内打开后继续支付。',
      wechatPaymentMpNotConfigured: '微信支付公众号配置不完整。',
      wechatScanOnDesktopHint: '桌面端请使用微信扫码支付。',
      wechatSwitchBrowserHint: '请切换到支持微信支付的浏览器。',
      wechatUnavailable: '当前微信支付不可用。'
    },
    recovery: {
      current: '当前支付恢复'
    },
    ...
    admin: {
      ...
      days: '天',
      daySuffix: '日',
      deductBalance: '扣除到账余额',
      deductBalanceHint: '退款时同步扣除用户已到账余额。',
      deletePlan: '删除套餐',
      deletePlanConfirm: '确认删除该套餐？',
      editPlan: '编辑套餐',
      expiresAt: '过期时间',
      features: '套餐权益',
      featuresHint: '每行一条权益。',
      featuresPlaceholder: '例如：更高并发\\n更多模型',
      feeRate: '手续费率',
      forceRefund: '强制退款',
      forSale: '上架销售',
      group: '分组',
  ...
  profile: {
    ...
    workbench: {
      ...
      eyebrow: '我的账户',
      introAriaLabel: '账户设置说明',
      introKicker: '账号与安全',
      introTitle: '管理你的登录信息和安全验证',
      introDescription: '这里只处理你的个人资料、密码和二次验证。余额、用量、充值和 API Key 已经拆到左侧对应入口，避免普通用户误进后台控制台。',
      basicInfoKicker: '基础资料',
      accountInfoTitle: '账号信息',
      displayNameKicker: '显示名称',
      editProfileTitle: '编辑个人资料',
      loginProtectionKicker: '登录保护',
      changePasswordTitle: '修改密码',
      twoFactorKicker: '二次验证',
      securityTitle: '账号安全'
    },
    accountBalance: '账户余额',
    ...
    avatar: {
      ...
      uploadTitle: '选择头像图片',
      uploadButton: '选择图片',
      uploadHint: '支持 JPEG、PNG、WebP，最大 5MB；也可以拖入图片',
      selectedFile: '已选择：{name}',
      zoomLabel: '缩放',
      cropHint: '头像会以图片中心为基准自动居中裁剪，并在浏览器中重新编码后上传。',
    ...
    totp: {
      ...
      statusLoadFailed: '双因素认证状态暂时无法加载',
      statusLoadFailedHint: '当前无法确认是否已启用，请刷新重试；这里不会把状态未知显示成未启用。',
      retryStatus: '重试',
      setupTitle: '设置双因素认证',
  ...
  admin: {
    apiKeyInventory: {
      title: '全站 API Key',
      description: '查看全站密钥状态、所属分组与实际消费',
      searchPlaceholder: '搜索名称、用户邮箱、ID 或 Key 尾号',
      userFilter: '筛选用户',
      allGroups: '全部分组',
      allStatuses: '全部状态',
      statusActive: '正常',
      ...
      summaryInactive: '已停用',
      summaryExpired: '已过期',
      summaryCost: '近 30 天实际消费',
      user: '所属用户 / 余额',
      key: '密钥',
      group: '分组',
      quota: '额度',
      todayCost: '今日消费',
      last30DaysCost: '近 30 天消费',
      totalCost: '累计消费',
      status: '状态',
      lastUsedAt: '最近使用',
      createdAt: '创建时间',
      ...
      createdShort: '创建',
      expiresShort: '过期',
      sortLastUsed: '排序：最近使用',
      sortTodayCost: '排序：今日消费',
      sortLast30DaysCost: '排序：近 30 天消费',
      sortTotalCost: '排序：累计消费',
      sortCreatedAt: '排序：创建时间',
      sortAscending: '升序',
      sortDescending: '降序',
      noGroup: '未分组',
      unlimited: '不限额',
      neverUsed: '从未使用',
      neverExpires: '永不过期',
      empty: '暂无符合条件的 API Key',
      loadFailed: '加载全站 API Key 失败',
      actions: '操作',
      enableAction: '启用',
      disableAction: '禁用',
      changeGroupAction: '改分组',
      deleteAction: '删除',
      changeGroupTitle: '修改 Key 分组',
      deleteTitle: '删除 API Key',
      deleteConfirm: '确定删除“{name}”吗？所属用户：{user}。删除后该 Key 将立即失效。',
      enableSuccess: 'API Key 已启用',
      disableSuccess: 'API Key 已禁用',
      changeGroupSuccess: 'API Key 分组已更新',
      deleteSuccess: 'API Key 已删除',
      mutationFailed: 'API Key 操作失败'
    },
    // Dashboard
    dashboard: {
      title: '管理控制台',
    ...
    channelMonitor: {
      ...
      advanced: {
        ...
        headerNameInvalid: '请求头名称无效：{name}',
        bodyMode: '请求体处理',
        bodyModeOff: '默认',
        bodyModeMerge: '合并',
        bodyModeReplace: '覆盖',
    ...
    subscriptions: {
      ...
      bulkAssign: {
        ...
        title: '批量分配订阅',
        users: '选择用户',
        searchPlaceholder: '按邮箱搜索并选择多个用户',
        selectedCount: '已选择 {count} 个用户',
        clearSelection: '清空选择',
        noUsersSelected: '搜索用户并加入本次批量分配。',
        removeUser: '移除用户',
        confirmTitle: '确认批量分配',
        confirmMessage: '将为 {count} 个用户分配“{group}”，有效期 {days} 天。已存在的相同订阅将按后端规则复用。',
        confirmAction: '确认分配',
        continue: '继续确认',
        success: '批量分配完成：新建 {created} 个，复用 {reused} 个',
        partialSuccess: '批量分配部分完成：成功 {success} 个，失败 {failed} 个',
        failed: '批量分配订阅失败'
      },
    ...
    accounts: {
      ...
      batchCredentials: {
        title: '批量更新凭据字段',
        selectedCount: '将更新已选择的 {count} 个账号',
        field: '更新字段',
        fieldHint: '仅开放后端允许的三个字段，不会修改 API Key、Base URL 或其他凭据。',
        value: '新值',
        valuePlaceholder: '输入新值',
        valuePrivacyHint: '当前值不会读取或回显；输入内容也不会出现在确认页。',
        clearValue: '清除此字段',
        fields: {
          accountUuid: '账号 UUID',
          orgUuid: '组织 UUID',
        ...
        confirmTitle: '确认批量更新',
        confirmMessage: '确认更新 {count} 个账号的“{field}”？为保护凭据，确认页不显示输入值。',
        confirmAction: '确认更新',
        continue: '继续确认',
        success: '成功更新 {count} 个账号',
        partialSuccess: '批量更新部分完成：成功 {success} 个，失败 {failed} 个',
        failed: '批量更新凭据字段失败'
      },
      bulkEdit: {
        title: '批量编辑账号',
  ...
  purchase: {
    ...
    notEnabledDesc: '当前暂未开放在线充值/订阅入口，请先使用已有额度或按页面提示的方式充值。',
    notConfiguredTitle: '充值/订阅链接未配置',
    notConfiguredDesc: '充值/订阅入口暂未完成配置，请先使用已有额度或按页面提示的方式充值。',
  ...
  availableChannels: {
    ...
    providerTabsLabel: '按供应商筛选',
    allProviders: '全部',
    groupSearchPlaceholder: '搜索分组...',
    groupSearchEmpty: '没有匹配的分组',
    groupDescriptionFallback: '暂无分组说明',
    cacheWriteShort: '写入',
    cacheReadShort: '读取',
    maxOutputShort: '输出',
    perRequestShort: '次',
