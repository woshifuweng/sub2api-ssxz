...
describe('Aether-derived home structure', () => {
  it('keeps the original three-provider section skeleton and native animated artwork', () => {
    ...
    expect(home).toContain('font-size: clamp(2.8rem, 6vw, 5rem);')
    ...
    expect(home).not.toContain('<li>Grok</li>')
    expect(home).not.toContain('Grok')
    expect(home).toContain("const fullText = '欢迎使用SSXZAI'")
    expect(home).toContain('AI 开发工具统一接入平台')
    expect(home).toContain('整合 Claude Code、Codex CLI、Gemini CLI 等多个 AI 编程助手')
    expect(home).toMatch(/<h1 class="aether-hero__brand"[^>]*>[\s\S]*?class="typewriter"/)
    expect(home).toContain('class="aether-hero__subtitle"')
    expect(home).toContain('class="aether-hero__description"')
    ...
    expect(sectionAnimations).toContain(
  ...
  it('draws the SSXZ vector artwork without falling back to the PNG', () => {
    ...
    expect(animatedLogo).toContain('ssxz-outline-retire .42s ease 1.92s forwards')
    expect(animatedLogo).toContain('to { opacity: 0; visibility: hidden; }')
    expect(animatedLogo).not.toContain('transform: scale(.992)')
