<template>
  <AppLayout>
            ...
            <template #cell-actions="{ row }">
              <div class="flex items-center gap-1">
                ...
                <LiquidButton
                  ...
                  class="inline-flex items-center gap-1 rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-900 dark:hover:bg-dark-700 dark:hover:text-white"
                  variant="plain"
                  size="sm"
                >
                  <svg
    ...
    />
    <BatchUpdateCredentialsDialog
      :show="showBatchCredentials"
      :account-ids="selIds"
      @close="showBatchCredentials = false"
      @updated="reload"
      @completed="handleBatchCredentialsCompleted"
    />
...
const showBatchCredentials = ref(false);
...
const ACCOUNT_SORTABLE_KEYS = new Set([
  ...
  'upstream_billing_rate', 'last_used_at', 'created_at', 'expires_at'
])
const loadInitialAccountSortState = (): AccountSortState => {
  ...
  try {
    ...
    return { sort_by: key, sort_order: parsed.order === 'desc' ? 'desc' : 'asc' }
  } catch {
    return fallback
  }
}
...
const usageBatchCache = new Map<
  number,
  { data: AccountUsageInfo; ts: number }
>();
...
const handleAccountUsageLoaded = (
  accountID: number,
...
const flushQueuedUsageBatch = async () => {
  ...
  const requestTokensByAccount = accountIDs.reduce<Record<string, number>>(
    (acc, accountID) => {
      acc[String(accountID)] =
        usageBatchRequestTokenByAccountId.value[String(accountID)] ?? 0;
      return acc;
    },
    {},
  );
  ...
  try {
    ...
    for (const accountID of accountIDs) {
      const key = String(accountID);
      if (
        (usageBatchRequestTokenByAccountId.value[key] ?? 0) !==
        requestTokensByAccount[key]
      ) {
        continue;
      }
...
const getSchedulerScoreRows = (account: Account): AccountSchedulerGroupScore[] => {
  ...
  if (account.scheduler_score != null) {
    const score = account.scheduler_score
    return [{
      group_id: null,
      base_score: score.base_score,
      sticky_score: score.sticky_score,
      sticky_score_infinity: score.sticky_score_infinity,
      sticky_weighted_enabled: score.sticky_weighted_enabled,
    }]
  }
  return []
}
...
watch(upstreamBillingNow, () => {
  ...
  if (lastUpstreamBillingSortRefreshMinute === minute) return
...
const refreshAccountsIncrementally = async () => {
  ...
  try {
    const result = await adminAPI.accounts.listWithEtag(
      ...
      toRaw(params) as Record<string, unknown>,
...
function getAccountModelsHint(account: Account): string {
  ...
  if (!extra) return "";
  const refreshError =
    typeof extra.models_refresh_error === "string"
      ? extra.models_refresh_error.trim()
      : "";
  if (refreshError) {
    return t("admin.accounts.modelsRefreshError", { message: refreshError });
  }
  const fetchedAt =
    typeof extra.models_fetched_at === "string" ? extra.models_fetched_at : "";
  if (!fetchedAt) return "";
  const fetchedModels = Array.isArray(extra.fetched_models)
    ? extra.fetched_models.length
    : 0;
...
function getOpenAICompactState(row: Account): OpenAICompactBadgeState | null {
...
function getOpenAICompactMeta(row: Account): { label: string; className: string; dotClass: string } | null {
...
const handleBatchCredentialsCompleted = () => {
  showBatchCredentials.value = false;
  clearSelection();
};
...
const accountMatchesCurrentFilters = (account: Account) => {
  ...
  if (params.oauth_type) {
    const oauthType =
      typeof account.credentials?.oauth_type === "string"
        ? account.credentials.oauth_type
        : "";
    if (oauthType !== params.oauth_type) return false;
  }
  if (params.tier_id) {
    const tierID =
      typeof account.credentials?.tier_id === "string"
        ? account.credentials.tier_id
        : "";
...
const handleExportData = async () => {
  ...
  try {
    ...
    const toastID = appStore.showToast(
      "info",
      t("admin.accounts.dataExportTaskQueued"),
      undefined,
      {
...
const patchAccountModelsMetadata = (
  accountID: number,
  updates: {
    models?: string[];
...
const handleRefreshModels = async (a: Account) => {
  ...
  try {
    ...
    appStore.showError(
      error?.message || t("admin.accounts.refreshModelsFailed"),
    );
  }
};
...
const proxyExpiryText = (proxy: AccountProxy): string => {
  const { key, params: labelParams } = proxyExpiryLabelKey(proxy.expires_at ?? null, proxy.status)
  return labelParams ? t(key, labelParams) : t(key)
}
