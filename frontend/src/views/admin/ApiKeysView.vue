<template>
  <AppLayout>
        ...
        <div class="flex flex-wrap items-start gap-3">
          ...
          <div ref="userFilterRef" class="relative w-full sm:w-64">
            ...
            >
              ...
              >
                <span class="min-w-0 truncate text-sm text-gray-800 dark:text-gray-100">{{ user.email }}</span>
                <span class="shrink-0 text-xs text-gray-400">#{{ user.id }}</span>
              </button>
              <p v-if="!userSearchLoading && userResults.length === 0" class="px-3 py-3 text-sm text-gray-500">
                {{ t('empty.noData') }}
              </p>
            </div>
          </div>
          ...
          <button
            ...
            :title="sortOrder === 'desc' ? t('admin.apiKeyInventory.sortDescending') : t('admin.apiKeyInventory.sortAscending')"
            @click="toggleSortOrder"
          >
            <Icon :name="sortOrder === 'desc' ? 'arrowDown' : 'arrowUp'" size="md" />
          </button>

          <button
            type="button"
            class="btn btn-secondary ml-auto px-3"
            :disabled="loading"
        ...
        >
          <template #cell-user="{ row }">
            <div class="w-full min-w-0 md:w-[136px] md:max-w-[136px]">
              <div class="truncate font-medium text-gray-950 dark:text-white" :title="row.user.email">
                {{ row.user.email }}
              </div>
              <div class="mt-1.5 flex items-center gap-2 text-[11px] text-gray-400 dark:text-dark-500">
                <span>#{{ row.user.id }}</span>
                <span v-if="row.user.username">{{ row.user.username }}</span>
                <span class="font-medium text-gray-600 dark:text-gray-400">{{ formatMoney(row.user.balance) }}</span>
              </div>
            </div>
          </template>
          ...
          <template #cell-key="{ row }">
            <div class="w-full min-w-0 md:w-[136px] md:max-w-[136px]">
              <div class="truncate font-medium text-gray-950 dark:text-white" :title="row.name">{{ row.name }}</div>
              <code class="mt-1.5 block text-[11px] text-gray-400 dark:text-dark-500">{{ row.key }}</code>
              <div class="mt-2.5 flex items-center justify-between gap-3 text-[11px] text-gray-400 dark:text-dark-500">
                <span>{{ t('admin.apiKeyInventory.quota') }}</span>
                <span class="font-medium text-gray-600 dark:text-gray-400">
                  {{ row.quota > 0 ? `${formatMoney(row.quota_used)} / ${formatMoney(row.quota)}` : t('admin.apiKeyInventory.unlimited') }}
                </span>
              </div>
              <div v-if="row.quota > 0" class="mt-1 h-1 overflow-hidden rounded-full bg-gray-100 dark:bg-dark-700">
                <div
                  class="h-full rounded-full bg-primary-500"
                  :style="{ width: `${Math.min(100, (row.quota_used / row.quota) * 100)}%` }"
                />
              </div>
            </div>
          </template>
          ...
          <template #cell-group="{ value }">
            ...
            />
            <span v-else class="text-sm text-gray-400">{{ t('admin.apiKeyInventory.noGroup') }}</span>
          </template>

          <template #cell-total_actual_cost="{ row }">
            <div class="w-full min-w-0 space-y-1.5 text-xs tabular-nums md:w-32">
              <div class="flex justify-between gap-3 text-gray-400 dark:text-dark-500">
                <span>{{ t('admin.apiKeyInventory.todayCost') }}</span>
                <span class="font-medium text-gray-800 dark:text-gray-200">{{ formatMoney(row.today_actual_cost) }}</span>
              </div>
              <div class="flex justify-between gap-3 text-gray-400 dark:text-dark-500">
                <span>{{ t('admin.apiKeyInventory.last30DaysCost') }}</span>
                <span class="font-medium text-gray-800 dark:text-gray-200">{{ formatMoney(row.last_30_days_actual_cost) }}</span>
              </div>
              <div class="flex justify-between gap-3 text-gray-500 dark:text-dark-400">
                <span>{{ t('admin.apiKeyInventory.totalCost') }}</span>
                <span class="font-semibold text-gray-950 dark:text-white">{{ formatMoney(row.total_actual_cost) }}</span>
              </div>
            </div>
          </template>
          ...
          <template #cell-last_used_at="{ row }">
            <div class="w-full min-w-0 space-y-2 md:w-32">
              <span :class="['badge inventory-status-badge', statusClass(row.status)]">{{ statusLabel(row.status) }}</span>
              <div class="text-[11px] text-gray-400 dark:text-dark-500">{{ formatCompactTimestamp(row.last_used_at) }}</div>
            </div>
          </template>

          <template #cell-created_at="{ row }">
            <div class="w-full min-w-0 space-y-1.5 text-[11px] text-gray-400 dark:text-dark-500 md:w-28">
              <div><span class="mr-2">{{ t('admin.apiKeyInventory.createdShort') }}</span>{{ formatDateOnly(row.created_at) }}</div>
              <div><span class="mr-2">{{ t('admin.apiKeyInventory.expiresShort') }}</span>{{ row.expires_at ? formatDateOnly(row.expires_at) : t('admin.apiKeyInventory.neverExpires') }}</div>
            </div>
          </template>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center justify-end gap-1.5">
              <button
                ...
                :title="row.status === 'active' ? t('admin.apiKeyInventory.disableAction') : t('admin.apiKeyInventory.enableAction')"
                :aria-label="row.status === 'active' ? t('admin.apiKeyInventory.disableAction') : t('admin.apiKeyInventory.enableAction')"
                @click="setKeyEnabled(row, row.status !== 'active')"
              >
                <Icon :name="row.status === 'active' ? 'xCircle' : 'checkCircle'" size="sm" />
              </button>
              <button
                type="button"
                class="inline-flex h-9 w-9 items-center justify-center rounded-md border border-gray-200 text-gray-500 transition-colors hover:border-gray-300 hover:bg-gray-50 hover:text-gray-900 disabled:cursor-not-allowed disabled:opacity-50 dark:border-dark-600 dark:text-dark-400 dark:hover:bg-dark-700 dark:hover:text-white"
          ...
          <template #empty>
            <div class="flex flex-col items-center py-4">
              ...
              <p class="font-medium text-gray-900 dark:text-gray-100">{{ t('admin.apiKeyInventory.empty') }}</p>
            </div>
          </template>
        </DataTable>
      </template>
    ...
    <ConfirmDialog
      ...
      :message="t('admin.apiKeyInventory.deleteConfirm', { name: deletingKey?.name || '', user: deletingKey?.user.email || '' })"
...
const sortOptions = computed<SelectOption[]>(() => [
  ...
  { value: 'today_actual_cost', label: t('admin.apiKeyInventory.sortTodayCost') },
  { value: 'last_30_days_actual_cost', label: t('admin.apiKeyInventory.sortLast30DaysCost') },
  { value: 'total_actual_cost', label: t('admin.apiKeyInventory.sortTotalCost') },
  { value: 'created_at', label: t('admin.apiKeyInventory.sortCreatedAt') },
])

const columns = computed<Column[]>(() => [
  { key: 'user', label: t('admin.apiKeyInventory.user'), class: 'w-[136px] max-w-[136px] !px-2' },
  { key: 'key', label: t('admin.apiKeyInventory.key'), class: 'w-[136px] max-w-[136px] !px-2' },
  { key: 'group', label: t('admin.apiKeyInventory.group'), class: 'w-56 max-w-56 !px-2' },
  { key: 'total_actual_cost', label: t('admin.apiKeyInventory.costBreakdown'), class: 'w-32 max-w-32 !px-2', sortable: true },
  { key: 'last_used_at', label: t('admin.apiKeyInventory.activity'), class: 'w-32 max-w-32 !px-2', sortable: true },
  { key: 'created_at', label: t('admin.apiKeyInventory.lifecycle'), class: 'w-28 max-w-28 !px-2', sortable: true },
  { key: 'actions', label: t('admin.apiKeyInventory.actions'), class: 'w-32 min-w-32 !px-2 text-right' },
])
...
const summaryMetrics = computed(() => [
  { key: 'total', label: t('admin.apiKeyInventory.summaryTotal'), value: summary.value.total.toLocaleString(), testId: 'summary-total' },
  { key: 'active', label: t('admin.apiKeyInventory.summaryActive'), value: summary.value.active.toLocaleString(), testId: 'summary-active' },
  { key: 'inactive', label: t('admin.apiKeyInventory.summaryInactive'), value: summary.value.inactive.toLocaleString(), testId: 'summary-inactive' },
  { key: 'expired', label: t('admin.apiKeyInventory.summaryExpired'), value: summary.value.expired.toLocaleString(), testId: 'summary-expired' },
  { key: 'cost', label: t('admin.apiKeyInventory.summaryCost'), value: formatMoney(summary.value.last_30_days_actual_cost), testId: 'summary-cost' },
])
...
const loadGroups = async () => {
  try {
    ...
    groupOptions.value = [{ value: null, label: t('admin.apiKeyInventory.allGroups') }]
  }
}
...
const handleSort = (key: string, order: 'asc' | 'desc') => {
  const allowed: AdminAPIKeySort[] = [
    'created_at', 'last_used_at', 'today_actual_cost', 'last_30_days_actual_cost', 'total_actual_cost',
  ]
...
const formatCompactTimestamp = (value?: string | null) => value
...
const setKeyEnabled = async (key: AdminAPIKeyListItem, enabled: boolean) => {
  ...
  try {
    ...
    appStore.showError(error?.message || t('admin.apiKeyInventory.mutationFailed'))
  } finally {
...
const handleOutsideClick = (event: MouseEvent) => {
  if (userFilterRef.value && !userFilterRef.value.contains(event.target as Node)) {
    showUserDropdown.value = false
  }
}

onMounted(() => {
  const userID = Number(Array.isArray(route.query.user_id) ? route.query.user_id[0] : route.query.user_id)
