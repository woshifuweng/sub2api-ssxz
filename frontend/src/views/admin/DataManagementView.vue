<template>
    ...
    <div class="card p-6">
      <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <LiquidButton
            type="button"
            :disabled="loadingSoraProfiles"
            @click="loadSoraS3Profiles"
            variant="outline"
            size="sm"
          >
            {{
              loadingSoraProfiles
                ? t("common.loading")
                : t("admin.settings.soraS3.reloadProfiles")
            }}
          </LiquidButton>
        </div>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full min-w-[1000px] text-sm">
          <thead>
            <tr
              class="border-b border-gray-200 text-left text-xs uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:text-gray-400"
            >
              <th class="py-2 pr-4">
          ...
          <tbody>
            <tr
              v-for="profile in soraS3Profiles"
            ...
            >
              ...
              <td class="py-3 text-xs">
                <div class="flex flex-wrap gap-2">
                  <LiquidButton
                    type="button"
                    @click="editSoraProfile(profile.profile_id)"
                    variant="outline"
                    size="sm"
                  >
                    {{ t("common.edit") }}
  ...
  <Teleport to="body">
    ...
    <Transition name="dm-drawer-panel">
      ...
      >
        <div
          class="flex items-center justify-between border-b border-gray-200 px-4 py-3 dark:border-dark-700"
        >
          <h4 class="text-sm font-semibold text-gray-900 dark:text-white">
            {{
              creatingSoraProfile
                ? t("admin.settings.soraS3.createTitle")
                : t("admin.settings.soraS3.editTitle")
            }}
          </h4>
          <LiquidButton
            type="button"
        ...
        <div class="flex-1 overflow-y-auto p-4">
          <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <input
              v-model="soraProfileForm.endpoint"
              class="input w-full"
              :placeholder="t('admin.settings.soraS3.endpoint')"
            />
            <input
              v-model="soraProfileForm.region"
              class="input w-full"
              :placeholder="t('admin.settings.soraS3.region')"
            />
            <input
              v-model="soraProfileForm.bucket"
              class="input w-full"
              :placeholder="t('admin.settings.soraS3.bucket')"
            />
            <input
              v-model="soraProfileForm.prefix"
              class="input w-full"
              :placeholder="t('admin.settings.soraS3.prefix')"
            />
            <input
              v-model="soraProfileForm.access_key_id"
              class="input w-full"
              :placeholder="t('admin.settings.soraS3.accessKeyId')"
            />
            <input
              ...
              class="input w-full"
              :placeholder="
                soraProfileForm.secret_access_key_configured
                  ? t('admin.settings.soraS3.secretConfigured')
                  : t('admin.settings.soraS3.secretAccessKey')
              "
            />
            <input
              v-model="soraProfileForm.cdn_url"
              class="input w-full"
              :placeholder="t('admin.settings.soraS3.cdnUrl')"
            />
            <div>
              <input
            ...
            >
              <input
                v-model="soraProfileForm.force_path_style"
                type="checkbox"
              />
              <span>{{ t("admin.settings.soraS3.forcePathStyle") }}</span>
            </label>
            <label
              v-if="creatingSoraProfile"
              class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 md:col-span-2"
            >
        ...
        <div
          class="flex flex-wrap justify-end gap-2 border-t border-gray-200 p-4 dark:border-dark-700"
        >
          <LiquidButton
            type="button"
            @click="closeSoraProfileDrawer"
          ...
          >
            {{
              testingSoraProfile
                ? t("common.loading")
                : t("admin.settings.soraS3.testConnection")
            }}
          </LiquidButton>
          <LiquidButton
            type="button"
            :disabled="savingSoraProfile"
            @click="saveSoraProfile"
            size="sm"
          >
            {{
              savingSoraProfile
                ? t("common.loading")
                : t("admin.settings.soraS3.saveProfile")
            }}
          </LiquidButton>
        </div>
      </div>
  ...
  try {
    if (creatingSoraProfile.value) {
      const created = await adminAPI.settings.createSoraS3Profile({
        ...
        default_storage_quota_bytes: Math.round(
          (soraProfileForm.value.default_storage_quota_gb || 0) *
            1024 *
            1024 *
            1024,
        ),
...
async function testSoraProfileConnection() {
  ...
  try {
    const result = await adminAPI.settings.testSoraS3Connection({
      profile_id: creatingSoraProfile.value
        ? undefined
        : selectedSoraProfileID.value,
      enabled: soraProfileForm.value.enabled,
      endpoint: soraProfileForm.value.endpoint,
    ...
    appStore.showSuccess(
      result.message || t("admin.settings.soraS3.testSuccess"),
    );
  } catch (error) {
    appStore.showError(
      (error as { message?: string })?.message || t("errors.networkError"),
...
async function removeSoraProfile(profileID: string) {
  if (
    !window.confirm(t("admin.settings.soraS3.deleteConfirm", { profileID }))
  ) {
    return;
  }
...
function newDefaultSoraS3ProfileForm(
  ...
  return {
    ...
    default_storage_quota_gb: Number(
      (quotaBytes / (1024 * 1024 * 1024)).toFixed(2),
    ),
  };
}
