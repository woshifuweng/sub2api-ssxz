<template>
  <AppLayout>
          ...
          <div class="flex flex-1 flex-wrap items-center justify-end gap-2">
            ...
            >
              ...
              {{
                maintenanceRunning
                  ? t("common.loading")
                  : t("admin.proxies.autoMaintenance.runNow")
              }}
            </LiquidButton>
            <LiquidButton
            ...
            <LiquidButton @click="showCreateModal = true" size="default">
          ...
          <DataTable :columns="columns" :data="proxies" :loading="loading">
            ...
            <template #cell-protocol="{ value }">
              <span
                ...
                :class="[
                  'badge',
                  value.startsWith('socks') ? 'badge-primary' : 'badge-gray',
                ]"
              >
                {{ value.toUpperCase() }}
              </span>
              ...
              <div class="flex items-center gap-1.5">
                ...
                <div class="relative">
                  ...
                  >
                    ...
                    >
                      <span
                        class="truncate font-mono text-gray-600 dark:text-gray-300"
                        >{{ fmt.label }}</span
                      >
                    </LiquidButton>
                  </div>
            ...
            <template #cell-auth="{ row }">
              ...
              >
                ...
                <LiquidButton
                  ...
                  type="button"
                  :aria-label="
                    visiblePasswordIds.has(row.id) ? '隐藏密码' : '显示密码'
                  "
            ...
            <template #cell-actions="{ row }">
              <div class="flex items-center gap-1">
                <LiquidButton
                  ...
                  class="inline-flex items-center gap-1 rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-emerald-50 hover:text-emerald-600 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-emerald-900/20 dark:hover:text-emerald-400"
                  variant="plain"
                  size="sm"
                >
                  <svg
                ...
                <LiquidButton
                  ...
                  class="inline-flex items-center gap-1 rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-blue-50 hover:text-blue-600 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-blue-900/20 dark:hover:text-blue-400"
                  variant="plain"
                  size="sm"
                >
                  <svg
    ...
    >
      ...
      <div class="mb-6 flex border-b border-gray-200 dark:border-dark-600">
        <LiquidButton
          ...
          size="sm"
        >
          <Icon name="plus" size="sm" class="inline" />
      ...
      >
        ...
        <div>
          ...
          <div class="relative">
            ...
            <LiquidButton
              type="button"
              :aria-label="createPasswordVisible ? '隐藏密码' : '显示密码'"
      ...
      <div class="space-y-5">
          ...
          <div class="mb-3 flex items-center justify-between gap-3">
            <div>
              <div class="text-sm font-medium text-gray-900 dark:text-white">
                {{ t("admin.proxies.autoMaintenance.runScopeTitle") }}
              </div>
              <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                {{
                  t("admin.proxies.autoMaintenance.runScopeHint", {
                    count: selectedCount,
                  })
                }}
              </div>
        ...
        <div class="grid gap-5 lg:grid-cols-[1.2fr,1fr]">
              ...
              >
                {{
                  editingMaintenanceId
                    ? t("admin.proxies.autoMaintenance.editPlan")
                    : t("admin.proxies.autoMaintenance.createPlan")
                }}
              </div>
              <div class="grid gap-3">
                <input
                  v-model.trim="maintenanceForm.name"
                  class="input w-full"
                  :placeholder="t('admin.proxies.autoMaintenance.planName')"
                />
                <input
                  v-model.trim="maintenanceForm.cron_expression"
                  class="input w-full"
                  :placeholder="
                    t('admin.proxies.autoMaintenance.cronPlaceholder')
                  "
                />
                <textarea
                  v-model="maintenanceForm.source_proxy_ids"
                  class="input min-h-[88px] w-full"
                  :placeholder="
                    t('admin.proxies.autoMaintenance.sourceProxyIdsPlaceholder')
                  "
                ></textarea>
                <div class="grid grid-cols-2 gap-3">
                  <input
                    v-model.number="maintenanceForm.max_results"
                    type="number"
                    min="1"
                    class="input w-full"
                    :placeholder="t('admin.proxies.autoMaintenance.maxResults')"
                  />
                  <input
                    v-model.number="maintenanceForm.max_failures_before_pause"
                    type="number"
                    min="1"
                    class="input w-full"
                    :placeholder="
                      t('admin.proxies.autoMaintenance.maxFailures')
                    "
                  />
                </div>
                <label
          ...
          >
            <div class="mb-3 flex items-center justify-between gap-3">
              <div class="text-sm font-medium text-gray-900 dark:text-white">
                {{ t("admin.proxies.autoMaintenance.plans") }}
              </div>
              <LiquidButton
                :disabled="maintenanceLoadingPlans"
                @click="loadMaintenancePlans"
                variant="outline"
                size="sm"
              >
                {{
                  maintenanceLoadingPlans
                    ? t("common.loading")
                    : t("common.refresh")
                }}
              </LiquidButton>
            ...
            <div v-else class="space-y-3">
              ...
              >
                <div class="flex items-start justify-between gap-3">
                  <div>
                    <div class="font-medium text-gray-900 dark:text-white">
                      {{
                        plan.name ||
                        t("admin.proxies.autoMaintenance.defaultPlanName")
                      }}
                    </div>
                    <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                      {{ t("admin.proxies.autoMaintenance.nextRun") }}:
                      {{ plan.next_run_at || "-" }}
                    </div>
                  </div>
                  <span
                    class="badge"
                    :class="plan.enabled ? 'badge-success' : 'badge-gray'"
                  >
                    {{
                ...
                <div class="mt-3 flex flex-wrap gap-2">
                  <LiquidButton
                    @click="editMaintenancePlan(plan)"
                    variant="outline"
                    size="sm"
                    >{{ t("common.edit") }}</LiquidButton
                  >
                  <LiquidButton
                    @click="toggleMaintenancePlan(plan, !plan.enabled)"
                    variant="outline"
                    size="sm"
                  >
                    {{
                  ...
                  <LiquidButton
                    @click="deleteMaintenancePlan(plan.id)"
                    variant="destructive"
                    size="sm"
                    >{{ t("common.delete") }}</LiquidButton
                  >
                  <LiquidButton
                    @click="loadMaintenanceResults(plan.id)"
                    variant="outline"
                    size="sm"
                    >{{
                      t("admin.proxies.autoMaintenance.results")
                    }}</LiquidButton
                  >
                </div>
              </div>
            ...
            <div
              v-if="maintenanceResults.length > 0"
              class="mt-5 border-t border-gray-200 pt-4 dark:border-dark-700"
            >
              <div
              ...
              <div class="space-y-2">
                <div
                  v-for="result in maintenanceResults"
                  :key="result.id"
                ...
                >
                  ...
                  <div class="mt-1 text-gray-500 dark:text-gray-400">
                    {{
                      t("admin.proxies.autoMaintenance.resultStats", {
                        checked: result.checked_proxies,
                        healthy: result.healthy_proxies,
                        failed: result.failed_proxies,
                        moved: result.moved_accounts,
                      })
                    }}
                  </div>
                </div>
    ...
    >
      <div v-if="qualityReport" class="space-y-4">
        ...
        >
          ...
          <div
            class="mt-3 grid grid-cols-2 gap-2 text-xs text-gray-600 dark:text-gray-300"
          >
            <div>
      ...
      <template #footer>
        <div class="flex justify-end">
          <LiquidButton
            @click="closeQualityReportDialog"
            variant="outline"
            size="sm"
          >
            {{ t("common.close") }}
    ...
    >
      ...
      <template #footer>
        <div class="flex justify-end">
          <LiquidButton @click="closeAccountsModal" variant="outline" size="sm">
            {{ t("common.close") }}
          </LiquidButton>
        </div>
      </template>
...
const loadMaintenanceResults = async (planId: number) => {
  try {
    maintenanceResults.value = await adminAPI.proxyMaintenance.listResults(
      planId,
      10,
    );
  } catch (error: any) {
...
const editMaintenancePlan = (plan: ProxyMaintenancePlan) => {
  ...
  maintenanceForm.max_failures_before_pause =
    plan.max_failures_before_pause || 3;
};
...
const saveMaintenancePlan = async () => {
  ...
  try {
    ...
    if (editingMaintenanceId.value) {
      await adminAPI.proxyMaintenance.updatePlan(
...
const toggleMaintenancePlan = async (
  plan: ProxyMaintenancePlan,
  enabled: boolean,
) => {
  try {
...
const handleAutoMaintenanceRun = async () => {
  ...
  try {
    const sourceProxyIds =
      selectedCount.value > 0 ? Array.from(selectedProxyIds.value) : [];
    const task = await adminAPI.proxyMaintenance.runNow(sourceProxyIds);
    const toastID = appStore.showToast(
      "info",
    ...
    appStore.showError(
      error?.message || t("admin.proxies.autoMaintenance.runFailed"),
    );
    maintenanceRunning.value = false;
  }
};
