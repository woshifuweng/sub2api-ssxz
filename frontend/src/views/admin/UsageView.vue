<template>
  <AppLayout>
    ...
    <div class="space-y-6 admin-b1-outline-scope">
      ...
      <UsageFilters ref="usageFiltersRef" v-model="filters" :start-date="startDate" :end-date="endDate" :exporting="exporting" @change="applyFilters" @refresh="refreshData" @reset="resetFilters" @cleanup="openCleanupDialog" @export="exportToExcel">
      ...
      <div v-if="filters.user_id" class="flex flex-wrap items-center gap-2 rounded-lg bg-primary-50 px-3 py-2 text-sm text-primary-700 dark:bg-primary-900/20 dark:text-primary-300">
        <span>当前仅查看用户 #{{ filters.user_id }} 的使用记录</span>
        <button type="button" class="rounded-md px-2 py-1 text-xs font-medium hover:bg-primary-100 dark:hover:bg-primary-900/40" @click="goToUserOrders(filters.user_id)">
...
const filters = ref<AdminUsageQueryParams>({ user_id: undefined, model: undefined, request_id: undefined, group_id: undefined, request_type: undefined, billing_type: null, start_date: startDate.value, end_date: endDate.value })
