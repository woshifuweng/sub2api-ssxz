  ...
  <AppLayout>
    ...
    <div class="space-y-6 admin-b1-outline-scope">
      ...
      <div class="card overflow-hidden">
        <div class="flex flex-wrap items-center border-b border-gray-200 px-2 dark:border-dark-700 sm:px-4">
          <LiquidButton
            ...
            class="-mb-px inline-flex items-center gap-1.5 rounded-none border-b-2 px-3 py-3 text-sm font-medium sm:px-4"
      ...
      <div
        ...
        class="flex flex-wrap items-center gap-2 rounded-lg bg-primary-50 px-3 py-2 text-sm text-primary-700 dark:bg-primary-900/20 dark:text-primary-300"
      >
        <span>当前仅查看用户 #{{ filters.user_id }} 的使用记录</span>
        <LiquidButton
          type="button"
          class="rounded-md px-2 py-1 text-xs font-medium hover:bg-primary-100 dark:hover:bg-primary-900/40"
          @click="goToUserOrders(filters.user_id)"
          variant="plain"
          size="sm"
        >
...
const detailTabs = computed<Array<{ key: DetailTab; label: string; icon: IconName }>>(() => [
  { key: "usage" as const, label: t("usage.tabs.usage"), icon: "document" },
  {
    key: "errors" as const,
    label: t("usage.tabs.errors"),
    icon: "exclamationTriangle",
  },
  { key: "ranking" as const, label: t("usage.tabs.ranking"), icon: "chart" },
]);
const rankingMounted = ref(false);
const rankingFilters = computed<Record<string, unknown>>(() => ({ ...filters.value }));
...
const toRFC3339 = (
  ...
  date
    ? new Date(
        `${date}${endOfDay ? "T23:59:59.999" : "T00:00:00"}`,
      ).toISOString()
    : undefined;
...
const switchTab = (tab: DetailTab) => {
  activeTab.value = tab;
  if (tab === "errors" && errRows.value.length === 0) void loadAdminErrors();
...
const onErrPage = (page: number) => {
  errPage.value = page;
  void loadAdminErrors();
};
const onErrPageSize = (pageSize: number) => {
  errPageSize.value = pageSize;
  errPage.value = 1;
  void loadAdminErrors();
};
...
const applyRouteQueryFilters = () => {
  ...
  const queryRequestId = getSingleQueryValue(route.query.request_id);
...
const loadRouteUserFilterLabel = async () => {
  ...
  try {
    ...
    usageFiltersRef.value?.setUserKeyword?.(
      user.email || String(requestedUserId),
    );
  } catch {
    if (!routeUserFilterIsCurrent()) return;
...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t("usage.sentUpstreamModel"),
      t("usage.upstreamResponseModel"),
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.upstream_model_mismatch == null
          ? ""
          : t(log.upstream_model_mismatch ? "common.yes" : "common.no"),
...
defineExpose({ requestedModelStats, refreshData, activeTab });
</script>
