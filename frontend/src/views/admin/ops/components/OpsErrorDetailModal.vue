            ...
            <div class="flex flex-wrap items-center justify-between gap-2">
              ...
              <div class="flex items-center gap-2">
                ...
                >
                  <Icon
                    :name="
                      expandedUpstreamDetailIds.has(
                        getUpstreamEventKey(ev, idx),
                      )
                        ? 'chevronDown'
                        : 'chevronRight'
                    "
            ...
            >
              <div>
                <span class="text-gray-400"
                  >{{ t("admin.ops.errorDetail.account") }}:</span
                >
                <span class="ml-1 font-mono">{{
                  getUpstreamEventAccountLabel(ev)
                }}</span>
              </div>
              <div>
                <span class="text-gray-400"
              ...
              <div>
                ...
                >
                <span class="ml-1 font-mono">{{
                  getUpstreamEventRequestId(ev) || "—"
                }}</span>
              </div>
              <div>
                <span class="text-gray-400"
                  >{{ t("admin.ops.errorDetail.time") }}:</span
                >
                <span class="ml-1 font-mono">{{
                  getUpstreamEventTime(ev)
                }}</span>
              </div>
            </div>

            <div
              v-if="getUpstreamEventMessage(ev)"
              class="mt-3 break-words text-sm font-medium text-gray-900 dark:text-white"
            >
              {{ getUpstreamEventMessage(ev) }}
            </div>
...
function isInlineUpstreamEvent(
  ev: UpstreamEventRow,
): ev is ParsedUpstreamEvent {
  return !("id" in ev);
}
