            ...
            <div class="space-y-5">
              ...
              <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                ...
                <label class="block">
                  ...
                  <input
                    v-model.number="form.order_timeout_minutes"
                    type="number"
                    min="1"
                    step="1"
                    class="input"
                ...
                <label class="block">
                  ...
                  <input
                    v-model.number="form.max_pending_orders"
                    type="number"
                    min="1"
                    step="1"
                    class="input"
                ...
                <label class="block">
                  ...
                  <input
                    v-model.number="form.balance_recharge_multiplier"
                    type="number"
                    min="0.01"
                    step="0.01"
                    class="input"
                ...
                <label class="block">
                  ...
                  <input
                    v-model.number="form.recharge_fee_rate"
                    type="number"
                    min="0"
                    max="100"
                    step="0.01"
              ...
              <div>
                ...
                <div class="mt-2 flex flex-wrap gap-2">
                  <label
                    ...
                    :class="
                      isPaymentTypeEnabled(option.value)
...
function togglePaymentType(type: string) {
  if (isPaymentTypeEnabled(type)) {
    form.enabled_payment_types = form.enabled_payment_types.filter(
      (item) => item !== type,
    );
    return;
  }
...
function configPayload(): UpdatePaymentConfigRequest {
  return {
    ...
    order_timeout_minutes: Math.round(
      positiveNumber(form.order_timeout_minutes, 30),
    ),
    ...
    balance_recharge_multiplier: positiveNumber(
...
async function reorderProviders(updates: { id: number; sort_order: number }[]) {
  try {
    await Promise.all(
      updates.map((item) =>
        adminPaymentAPI.updateProvider(item.id, {
