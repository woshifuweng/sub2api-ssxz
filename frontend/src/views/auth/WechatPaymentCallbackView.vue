...
function normalizeRedirectPath(path: string | null | undefined): string {
  ...
  if (!value) return '/app/purchase'
  if (!value.startsWith('/')) return '/app/purchase'
  if (value.startsWith('//') || value.includes('://')) return '/app/purchase'
  if (value === '/payment') return '/app/purchase'
  if (value.startsWith('/payment?')) return '/app/purchase' + value.slice('/payment'.length)
  return value
}
