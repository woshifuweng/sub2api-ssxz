...
onMounted(async () => {
  try {
    ...
    const keys = res.items ?? []          // NOTE: .items, NOT .data
    const first = keys[0]
    if (first?.id) {
...
.workbench-frame { flex: 1; border: none; width: 100%; height: 100%; display: block; }
.workbench-placeholder {
  flex: 1; display: flex; flex-direction: column;
  align-items: center; justify-content: center;
  gap: 1rem; color: var(--ssxz-text-muted); font-size: 0.9rem;
}
.workbench-spinner { width: 2rem; height: 2rem; opacity: 0.4; animation: spin 1s linear infinite; }
.workbench-empty-icon { width: 3rem; height: 3rem; opacity: 0.3; }
.workbench-key-link { color: var(--ssxz-accent, #6366f1); font-size: 0.85rem; text-decoration: none; }
