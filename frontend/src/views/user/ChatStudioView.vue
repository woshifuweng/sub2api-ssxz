<template>
  <div
    class="mx-auto grid max-w-7xl gap-5 xl:grid-cols-[280px_minmax(0,1fr)_320px]"
  >
    <aside class="space-y-4">
      ...
      >
        <div class="mb-4 flex items-center justify-between gap-3">
          ...
          <LiquidButton type="button" @click="createSession(mode)" size="sm">
      ...
      <section
        class="rounded-lg border border-gray-200 bg-white p-3 shadow-sm dark:border-dark-600 dark:bg-dark-800"
      >
        <div class="mb-2 flex items-center justify-between">
          ...
          <LiquidButton
            type="button"
            class="text-xs text-gray-500 hover:text-primary-600 dark:text-dark-300"
        ...
        <div class="max-h-[360px] space-y-2 overflow-y-auto pr-1">
          ...
          >
            ...
            <span class="mt-1 block text-xs text-gray-500 dark:text-dark-400">
              {{ session.mode === "commerce" ? "电商文案" : "普通聊天" }} ·
    ...
    <section
      class="flex min-h-[760px] flex-col rounded-lg border border-gray-200 bg-white shadow-sm dark:border-dark-600 dark:bg-dark-800"
    >
      <header
        class="flex flex-col gap-3 border-b border-gray-200 px-5 py-4 dark:border-dark-600 lg:flex-row lg:items-center lg:justify-between"
      >
        <div>
      ...
      >
        ...
        >
          <div class="max-w-md text-center">
            <div
              class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-lg bg-primary-50 text-primary-600 dark:bg-primary-900/30 dark:text-primary-300"
            >
              <Icon
                :name="mode === 'commerce' ? 'sparkles' : 'chat'"
                size="lg"
              />
            </div>
        ...
        <article v-if="sending" class="flex justify-start">
          <div
            class="rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-500 dark:border-dark-600 dark:bg-dark-900/60 dark:text-dark-300"
          >
            正在生成...
          </div>
        </article>
      ...
      <footer class="border-t border-gray-200 p-4 dark:border-dark-600">
        ...
        <div
          v-if="mode === 'chat'"
          class="rounded-lg border border-gray-200 bg-gray-50 p-2 dark:border-dark-600 dark:bg-dark-900/60"
        >
          <textarea
            v-model="draft"
        ...
        >
          <p class="text-xs text-gray-500 dark:text-dark-400">
            当前使用
            {{ selectedModel }}，后台会根据所选电商场景自动套用专业提示词模板。
          </p>
          <div class="flex gap-2">
            <LiquidButton
              type="button"
              @click="
                copyText(buildCommerceVisibleRequest(buildCommerceContext()))
              "
              variant="outline"
              size="sm"
            >
    ...
    <aside class="space-y-5">
      ...
      >
        ...
        <div class="mt-4 space-y-4">
          ...
          <div>
            ...
            <input
              v-model="commerceForm.productName"
              class="input"
              placeholder="例如：蓝牙耳机"
            />
          </div>
          ...
          <div class="grid grid-cols-2 gap-2">
            <div>
              ...
              <select v-model="commerceForm.platform" class="input">
                <option
                  v-for="item in commercePlatforms"
                  :key="item"
                  :value="item"
                >
                  {{ item }}
            ...
            <div>
              ...
              <select v-model="commerceForm.tone" class="input">
                <option v-for="item in commerceTones" :key="item" :value="item">
                  {{ item }}
                </option>
              </select>
            </div>
          ...
          <div>
            ...
            <input
              v-model="commerceForm.audience"
              class="input"
              placeholder="例如：通勤上班族、学生、运动人群"
            />
          </div>
      ...
      >
        ...
        <div class="mt-3 space-y-2 text-sm text-gray-600 dark:text-dark-300">
          <router-link
            to="/app/image"
            class="block rounded-lg bg-gray-50 px-3 py-2 hover:bg-primary-50 dark:bg-dark-700 dark:hover:bg-primary-900/30"
          >
            商品作图和上传改图
          </router-link>
          <router-link
...
const quickPrompts = [
  {
    ...
    desc: "生成标题、卖点和详情页结构",
    prompt:
      "帮我为一款蓝牙耳机写电商详情页文案，突出降噪、长续航、轻便，语气专业、适合商用。",
  },
  {
    mode: "chat",
    title: "客服回复",
...
const mode = ref<WorkMode>(
  route.query.mode === "ecommerce" ? "commerce" : "chat",
);
const draft = ref("");
...
const visibleQuickPrompts = computed(() =>
  quickPrompts.filter((item) => item.mode === mode.value),
);
const activeModel = computed(() =>
  models.find((item) => item.id === selectedModel.value),
);
const selectedCommerceScene = computed(() =>
  commerceScenes.find((item) => item.value === commerceForm.outputGoal),
);
const workspaceTitle = computed(() =>
  mode.value === "commerce"
    ? "电商文案工作台"
    : activeModel.value?.name || "AI 对话",
);
const emptyTitle = computed(() =>
  mode.value === "commerce" ? "填写右侧表单，生成商用文案" : "直接输入问题开始",
);
...
const canGenerateCommerce = computed(
  () =>
    commerceForm.productName.trim() !== "" ||
    commerceForm.sellingPoints.trim() !== "",
);
const canRegenerate = computed(() =>
  currentMessages.value.some((message) => message.role === "user"),
);
...
function resetActiveSession() {
  ...
  activeSession.value.title =
    mode.value === "commerce" ? "新的电商文案" : "新的聊天";
  errorMessage.value = "";
  touchSession();
}
...
function touchSession() {
  ...
  const firstUser = activeSession.value.messages.find(
    (message) => message.role === "user",
  );
  if (firstUser?.content) {
    activeSession.value.title = firstUser.content
      .replace(/\s+/g, " ")
      .slice(0, 28);
  }
  sessions.value = [
...
async function sendMessagesToGateway(
...
function outputGoalLabel(value: string) {
  return (
    commerceScenes.find((item) => item.value === value)?.label || "完整电商套装"
  );
}
