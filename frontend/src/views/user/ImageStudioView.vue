  ...
  >
    <section class="image-hero">
      <div>
        ...
        <p>这里用于内测图片生成、历史预览和下载，不承诺正式图片产品能力。</p>
        <div class="hero-note">
        ...
        <div class="hero-actions" aria-label="作图辅助入口">
          ...
          <span
            >图片入口当前是内测能力，对话只辅助整理提示词，不代表正式作图主流程。</span
          >
        </div>
      </div>
      <div class="hero-side">
        <div class="hero-route-card">
          ...
          <small
            >适合整理卖点、标题、画面气质和提示词，不把站点做成纯作图。</small
          >
        </div>
        ...
        <div class="hero-model-card" aria-label="可用图片模型">
          ...
          <div v-if="imageModelPreview.length" class="hero-model-list">
            <span v-for="model in imageModelPreview" :key="model.id">{{
              model.name
            }}</span>
            <span v-if="hiddenImageModelCount > 0"
              >+{{ hiddenImageModelCount }}</span
            >
          </div>
        </div>
      </div>
    ...
    <section class="image-workbench" aria-label="图片内测入口">
      <aside class="creation-console" aria-label="创作控制台">
        <div class="console-scroll">
          ...
          <section class="console-block">
            <div class="block-heading">
              ...
              <div>
                <h3>选择画幅比例</h3>
                <p>
                  可选常用电商比例，也可以填自定义比例；当前模型会按最接近尺寸生成。
                </p>
              </div>
            </div>
            ...
            <div v-if="canvasId === 'custom'" class="sub-panel">
              <div class="ratio-input-row">
                <label class="inline-field">
                  <span>宽</span>
                  <input
                    v-model.trim="customRatioWidth"
                    class="control-input"
                    inputmode="decimal"
                    placeholder="例如：2 或 1080"
                  />
                </label>
                ...
                <label class="inline-field">
                  <span>高</span>
                  <input
                    v-model.trim="customRatioHeight"
                    class="control-input"
                    inputmode="decimal"
                    placeholder="例如：3 或 1350"
                  />
                </label>
              </div>
              <label class="inline-field">
                <span>画幅用途说明</span>
                <input
                  v-model.trim="customCanvasPurpose"
                  class="control-input"
                  placeholder="例如：朋友圈长图、店铺头图、课程封面"
                />
              </label>
          ...
          <section class="console-block">
            ...
            <label
              class="asset-drop"
              :class="{
                filled: Boolean(selectedFile),
                dragging: referenceDragging,
              }"
            ...
            >
              ...
              <template v-else>
                ...
                <span class="asset-copy">
                  <strong class="asset-title" :title="selectedFile.name">{{
                    selectedFile.name
                  }}</strong>
            ...
            <p v-if="referencePreviewError" class="reference-upload-note error">
              {{ referencePreviewError }}
            </p>
            <LiquidButton
              v-if="selectedFile"
              type="button"
              class="secondary-button mt-2 w-full"
          ...
          <section class="console-block">
            ...
            <label class="inline-field">
              ...
              <input
                ...
                class="control-input"
                placeholder="例如：无线耳机、护肤精华、咖啡杯、运动水杯"
              />
            </label>
            <label class="inline-field">
              <span>自有品牌名（可选）</span>
              <input
                ...
                class="control-input"
                placeholder="如需品牌露出，请填写自有品牌名；未提供时默认无品牌原创图。"
              />
            </label>
            <label class="inline-field">
              <span>画面描述</span>
          ...
          <section class="console-block">
            ...
            <label class="inline-field">
              ...
              <input
                v-model.trim="scene"
                class="control-input"
                placeholder="例如：晨光办公桌、极简白棚、户外露营、浴室台面"
              />
            </label>
            ...
            <label v-if="style === customStyleOption" class="inline-field">
              <span>自定义风格描述</span>
              <input
                v-model.trim="customStyle"
                class="control-input"
                placeholder="例如：复古杂志感、奢侈品广告风、科技蓝银质感"
              />
            </label>
              ...
              <div class="advanced-body">
                <label class="inline-field">
                  ...
                  <input
                    ...
                    class="control-input"
                    placeholder="例如：高清细节、无水印、不要乱码文字、留白构图"
                  />
                </label>
              </div>
            </details>
        ...
        <div class="console-action">
          ...
          <p class="safety-note">
            AI
            生成图片仅供参考，商用前请确认没有第三方品牌、Logo、版权素材或不真实商品信息。
          </p>
          <LiquidButton
            type="button"
      ...
      <section ref="previewStageRef" class="canvas-panel" aria-label="作品画布">
        ...
        <div class="canvas-stage">
          <div class="canvas-sheet" :style="canvasSheetStyle">
            <img
              ...
              :alt="`result-${activeResultIndex + 1}`"
            />
            <div v-else class="canvas-empty" :class="{ failed: errorMessage }">
              <span class="canvas-empty-badge">{{
                errorMessage ? "需要处理" : "等待创作"
              }}</span>
              <Icon
                :name="
                  generating
        ...
        <div class="result-actions">
          ...
          <LiquidButton
            type="button"
            class="secondary-button"
            :disabled="!activeResult || generating"
            @click="generate"
            variant="plain"
            size="sm"
          >
    ...
    <section class="recent-works">
      <div class="recent-heading">
        ...
        <LiquidButton
          type="button"
          class="secondary-button"
          :disabled="recentWorksLoading"
          @click="loadRecentWorks"
          variant="plain"
          size="sm"
        >
          <Icon
            name="refresh"
            size="sm"
            :class="{ 'animate-spin': recentWorksLoading }"
          />
          刷新
        </LiquidButton>
      </div>
      ...
      <div
        v-if="recentWorksLoading && !recentWorks.length"
        class="recent-empty"
      >
        正在加载最近作品...
      </div>
    ...
    >
      ...
      >
        <header class="recent-preview-header">
          ...
          <LiquidButton
            type="button"
            class="recent-preview-close"
            aria-label="关闭预览"
            @click="closeWorkPreview"
            variant="plain"
            size="sm"
          >
            ×
        ...
        <div class="recent-preview-stage">
          <img
            :src="previewWork.src"
            :alt="`preview-work-${previewWork.work.id}`"
          />
        </div>
...
} from "@/composables/useUserCapabilities";
...
const canvasPresets: Array<{
  id: Exclude<CanvasId, "custom">;
  label: string;
  ratio: string;
  width: number;
  height: number;
...
const styleToPrompt: Record<string, string> = {
  现代极简:
    "modern minimal commercial photography, clean composition, restrained palette",
  高级商业摄影:
    "premium commercial photography, refined lighting, realistic texture, high-end composition",
  清透白底:
    "clean white background, soft light, sharp product edges, ecommerce ready",
  生活方式场景:
    "warm lifestyle scene, realistic environment, natural props, soft daylight",
  自然柔光:
    "soft natural light, airy composition, gentle shadows, premium texture",
  低饱和质感:
    "low saturation premium texture, muted color palette, editorial product photography",
  科技感:
    "sleek technology advertising style, cool lighting, glass and metal texture",
  国潮质感:
    "modern Chinese visual style, elegant cultural detail, premium commercial layout",
  奢华质感:
    "luxury advertising style, elegant materials, dramatic yet clean lighting",
};
...
const noImageModelMessage =
  "当前没有可用的图片生成模型。请确认账号可用范围后再使用图片生成。";
const failureRecoveryNote =
  "本次没有生成成功作品，不会保存到历史；未成功返回结果不会扣生成费用。可以调整提示词后重试。";
...
const selectedGoal = computed(
  () =>
    creationGoals.find((item) => item.id === goal.value) || creationGoals[0],
);
const selectedCanvas = computed(() => {
  if (canvasId.value === "custom") return customCanvas.value;
  return (
    canvasPresets.find((item) => item.id === canvasId.value) || canvasPresets[0]
  );
});
...
const isCustomRatioValid = computed(() => {
  ...
  return (
    isPositiveRatioPart(customRatioWidth.value) &&
    isPositiveRatioPart(customRatioHeight.value)
  );
});
...
const displayGoalLabel = computed(() =>
  goal.value === "custom"
    ? customGoal.value || "自定义创作"
    : selectedGoal.value.label,
);
const displayCanvasRatio = computed(() =>
  canvasId.value === "custom"
    ? customRatioValue.value
      ? `自定义 ${customRatioValue.value}`
      : "自定义比例"
    : selectedCanvas.value.ratio,
);
const displayStyleLabel = computed(() =>
  style.value === customStyleOption
    ? customStyle.value || "自定义风格"
    : style.value,
);
const requestStyle = computed(() => {
  if (style.value === customStyleOption)
    return customStyle.value || "custom user-defined commercial visual style";
  return styleToPrompt[style.value] || style.value;
});
const requestSize = computed(() => {
  if (selectedCanvas.value.width === selectedCanvas.value.height)
    return "1024x1024";
  return selectedCanvas.value.width > selectedCanvas.value.height
    ? "1536x1024"
    : "1024x1536";
});
const hasDescription = computed(() =>
  Boolean(productName.value.trim() || prompt.value.trim()),
);
const imageGenerationUnavailable = computed(
  () => capabilities.loaded.value && !activeImageModel.value,
);
const isGenerateDisabled = computed(
  () =>
    generating.value ||
    !hasDescription.value ||
    !isCustomRatioValid.value ||
    imageGenerationUnavailable.value,
);
...
const hiddenImageModelCount = computed(() =>
  Math.max(0, imageModelOptions.value.length - imageModelPreview.value.length),
);
const activeImageModel = computed(() => {
  if (selectedImageModelId.value) {
    const selected = imageModelOptions.value.find(
      (model) => model.id === selectedImageModelId.value,
    );
    if (selected) return selected;
  }
  const preferred = resolvePreferredImageModelId(
    imageModelOptions.value,
    capabilities.defaultImageModel.value,
  );
  return (
    imageModelOptions.value.find((model) => model.id === preferred) || null
  );
});
const activeImageModelLabel = computed(
  () => activeImageModel.value?.name || "系统配置",
);
const activeImageModelRouteLabel = computed(() =>
  imageModelRouteLabel(activeImageModel.value),
);
const imageModelHint = computed(() => {
  ...
  if (activeImageModel.value) {
    ...
    if (routeLabel)
      return `${routeLabel}；来自账号可用渠道，当前生成仍以系统配置为准。`;
...
const referenceMeta = computed(() => {
  ...
  const ext =
    selectedFile.value.name.split(".").pop()?.toUpperCase() ||
    fileTypeLabel(selectedFile.value.type);
...
const fullPrompt = computed(() =>
  [
    ...
    brandName.value
      ? `自有品牌名：${brandName.value}`
      : "品牌策略：未提供自有品牌时默认无品牌原创图",
...
watch(
  ...
  (models) => {
    ...
    if (
      selectedImageModelId.value &&
      models.some((model) => model.id === selectedImageModelId.value)
    ) {
      return;
    }
    selectedImageModelId.value = resolvePreferredImageModelId(
      models,
      capabilities.defaultImageModel.value,
    );
  },
...
function resolvePreferredImageModelId(
  models: Array<{ id: string }>,
  fallbackModelId = "",
) {
  if (models.some((model) => model.id === verifiedDefaultImageModelId))
    return verifiedDefaultImageModelId;
...
function isOpenAICompatibleImageRoute(model: ChatModelOption) {
  ...
  return (
    ...
    providerLabel.includes("openai-compatible")
  );
}
...
function handlePreviewImageError() {
  ...
  previewImageFailed.value = false;
  referencePreviewError.value =
    "参考图预览加载失败，请重新上传 JPG / PNG / WEBP 图片。";
  appStore.showError(referencePreviewError.value);
}
...
async function requestImageStudio() {
  ...
  const response = await apiClient.post<ImageStudioPayload>(
...
function normalizeUnknownError(error: unknown) {
  if (axios.isAxiosError(error)) {
    ...
    const raw =
      ...
      payload?.detail ||
      error.message;
    return normalizeImageError(
      String(raw || "生成失败，请稍后重试"),
...
function scrollPreviewIntoView() {
  previewStageRef.value?.scrollIntoView?.({
    behavior: "smooth",
    block: "start",
  });
}
...
.image-hero {
  ...
  background:
    linear-gradient(
      135deg,
      color-mix(in srgb, var(--ssxz-primary) 8%, var(--ssxz-surface)) 0%,
      var(--ssxz-surface) 52%
    ),
    var(--ssxz-surface);
}
...
.image-hero::after {
  ...
  background: linear-gradient(
    135deg,
    transparent 0 46%,
    color-mix(in srgb, var(--ssxz-primary) 12%, transparent) 46% 54%,
    transparent 54% 100%
  );
  opacity: 0.55;
...
.hero-model-card {
  width: 100%;
  border: 1px solid
    color-mix(in srgb, var(--ssxz-action) 36%, var(--ssxz-border));
  border-radius: 1rem;
  background:
    linear-gradient(
      135deg,
      color-mix(in srgb, var(--ssxz-action-soft) 72%, transparent),
      transparent 68%
    ),
    var(--ssxz-surface-subtle);
...
.step-dot {
  ...
  background: linear-gradient(
    135deg,
    var(--ssxz-action-soft),
    color-mix(in srgb, var(--ssxz-action) 12%, transparent)
  );
  color: var(--ssxz-action);
  font-size: 0.8rem;
  font-weight: 800;
...
.style-chip.selected {
  ...
  background: linear-gradient(
    135deg,
    color-mix(in srgb, var(--ssxz-action-soft) 80%, var(--ssxz-surface)),
    var(--ssxz-active)
  );
  box-shadow:
    0 0 0 3px var(--ssxz-focus-ring),
    var(--ssxz-shadow);
}
...
.asset-drop {
  ...
  background: linear-gradient(
    135deg,
    color-mix(in srgb, var(--ssxz-action-soft) 46%, var(--ssxz-surface-subtle)),
    var(--ssxz-surface-subtle)
  );
}
...
.console-action {
  ...
  background: linear-gradient(
    180deg,
    color-mix(in srgb, var(--ssxz-surface-elevated) 88%, transparent),
    var(--ssxz-surface-elevated)
  );
...
.generate-button {
  ...
  background: linear-gradient(
    135deg,
    var(--ssxz-action),
    color-mix(in srgb, var(--ssxz-action) 76%, #0f766e)
  );
  color: var(--ssxz-action-text);
  font-weight: 800;
}
...
.canvas-panel {
  ...
  background: linear-gradient(
    180deg,
    color-mix(in srgb, var(--ssxz-surface-raised) 94%, transparent),
    var(--ssxz-surface)
  );
  padding: 1rem;
}
...
.canvas-sheet {
  ...
  background:
    linear-gradient(
      45deg,
      color-mix(in srgb, var(--ssxz-border) 28%, transparent) 25%,
      transparent 25% 75%,
      color-mix(in srgb, var(--ssxz-border) 28%, transparent) 75%
    ),
    linear-gradient(
      45deg,
      color-mix(in srgb, var(--ssxz-border) 18%, transparent) 25%,
      transparent 25% 75%,
      color-mix(in srgb, var(--ssxz-border) 18%, transparent) 75%
    ),
    radial-gradient(
      circle at 50% 20%,
      var(--ssxz-glow-subtle),
      transparent 42%
    ),
    var(--ssxz-canvas);
  background-position:
