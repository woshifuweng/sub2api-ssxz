...
const baseAmount = computed(() => {
  if (!order.value || order.value.fee_rate <= 0)
    return order.value?.pay_amount ?? 0;
  return (
    Math.round(
      (order.value.pay_amount / (1 + order.value.fee_rate / 100)) * 100,
    ) / 100
  );
});
...
async function resolveOrderFromResumeToken(
  resumeToken: string,
): Promise<PublicPaymentOrder | null> {
  try {
    const result =
