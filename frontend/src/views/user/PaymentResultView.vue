    ...
    <div class="w-full max-w-md space-y-6">
      ...
      <template v-else>
        ...
        <div class="text-center">
          ...
          <div
            v-else-if="isMissingPaymentContext"
            class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-primary-100 dark:bg-primary-900/30"
          >
            <svg
              class="h-10 w-10 text-primary-600 dark:text-primary-300"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              stroke-width="2"
        ...
        <div class="flex gap-3">
          <LiquidButton
            class="flex-1"
            @click="router.push('/app/purchase')"
            variant="outline"
            size="sm"
            >{{ t("payment.result.backToRecharge") }}</LiquidButton
          >
          <LiquidButton
            class="flex-1"
            @click="router.push('/app/orders')"
            size="sm"
            >{{ t("payment.result.viewOrders") }}</LiquidButton
          >
        </div>
      </template>
    </div>
...
type PaymentResultOrder = PaymentOrder | PublicOrderVerifyResult;
...
const SETTLING_STATUSES = new Set<string>();
...
const baseAmount = computed(() => {
  ...
  return (
    Math.round(
      (order.value.pay_amount / (1 + feeRate / 100)) * 100,
    ) / 100
  );
});
...
function hasPaymentType(nextOrder: PaymentResultOrder | null): nextOrder is PaymentOrder {
  return !!nextOrder
    && "payment_type" in nextOrder
    && typeof nextOrder.payment_type === "string"
    && nextOrder.payment_type.trim() !== "";
}
...
function isRefreshablePaymentStatus(
...
async function resolveOrderFromResumeToken(
  resumeToken: string,
): Promise<PaymentResultOrder | null> {
  try {
    const result =
...
function scheduleStatusRefresh(
  refreshOrder: (() => Promise<PaymentResultOrder | null>) | null,
): void {
  clearStatusRefreshTimer();
  if (
    !refreshOrder ||
    !isRefreshablePaymentStatus(order.value?.status) ||
...
onMounted(async () => {
  ...
  hasPaymentLookupContext.value = Boolean(
    resumeToken || routeOrderId > 0 || outTradeNo || hasLegacyFallbackContext,
  );
