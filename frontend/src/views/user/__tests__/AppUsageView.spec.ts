...
const messages: Record<string, string> = {
  ...
  'usage.workbench.dailyCostTrend': 'Daily actual spend',
  'usage.workbench.dailyRequestTrend': 'Daily requests',
  'usage.workbench.dailyTokenTrend': 'Daily token usage',
  'usage.workbench.actualChargeTrendHint': 'Final balance deductions only',
  'usage.workbench.requestTrendHint': 'Successful records grouped by day',
  'usage.workbench.tokenTrendHint': 'Input, output, and cache tokens combined',
  'usage.workbench.period7Days': 'Past 7 days',
  'usage.workbench.period30Days': 'Past 30 days',
...
vi.mock('vue-chartjs', () => ({
  Line: {
    name: 'Line',
    props: ['data'],
    template: '<div data-testid="usage-metric-line" :data-values="data.datasets[0].data.join(\',\')" />'
  },
  Bar: {
    name: 'Bar',
    props: ['data'],
    template: '<div data-testid="usage-metric-bar" :data-values="data.datasets[0].data.join(\',\')" />'
  }
}))
...
describe('AppUsageView', () => {
  ...
  it('renders usage data inside the new workbench page instead of the old usage UI', async () => {
    ...
    expect(text).toContain('Daily actual spend')
    expect(text).toContain('Daily requests')
