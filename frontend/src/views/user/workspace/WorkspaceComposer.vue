<template>
  ...
  >
    <div
      v-if="assetPreviews.length"
      class="attachment-preview-list"
      data-testid="workspace-asset-previews"
    >
      <article
        v-for="asset in assetPreviews"
        :key="asset.id"
    ...
    <div class="composer-tool-row">
      ...
      <LiquidButton
        ...
        :class="{
          ...
          'is-active':
            tool.key === 'web-search' && tool.available && webSearchEnabled,
        }"
      ...
      >
        ...
        <small
          v-if="tool.key === 'web-search' && tool.available && webSearchEnabled"
          >已启用</small
        >
...
const capabilityTools = computed<
  Array<{
...
>(() => {
  ...
  return props.webSearchAvailable === true
    ? tools
    : tools.filter((tool) => tool.key !== "web-search");
});

const placeholder = computed(() =>
  props.imageCapabilityAvailable
    ? placeholders[props.intent]
    : placeholders.chat,
);
...
const sendButtonTitle = computed(() => {
  ...
  if (props.assetPreviews.length > 0)
    return "当前对话页暂不支持发送图片或文件，请移除附件后发送文字";
